...
func addGitHubConnections(m *integrations.Manager, logger *logging.Logger) {
	if token := os.Getenv("GITHUB_TOKEN"); token != "" {
		...
		_ = m.Add(name, github.NewClient(github.Config{
			Token:   token,
	...
	for i := 2; i <= 9; i++ {
		...
		_ = m.Add(name, github.NewClient(github.Config{Token: token, BaseURL: baseURL}, logger))
	}
}
...
func addBitbucketConnections(m *integrations.Manager, logger *logging.Logger) {
	...
	if user != "" && pass != "" {
		...
		_ = m.Add(name, bitbucket.NewClient(bitbucket.Config{
...
func addGitLabConnections(m *integrations.Manager, logger *logging.Logger) {
	if token := os.Getenv("GITLAB_TOKEN"); token != "" {
		...
		_ = m.Add(name, gitlab.NewClient(gitlab.Config{
			Token:   token,
	...
	for i := 2; i <= 9; i++ {
		...
		_ = m.Add(name, gitlab.NewClient(gitlab.Config{Token: token, BaseURL: baseURL}, logger))
	}
}
