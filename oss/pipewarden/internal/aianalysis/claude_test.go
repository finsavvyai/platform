...
func TestParseClaudeResponse_ValidJSON(t *testing.T) {
	...
	if findings[0].Severity != analysis.SeverityMedium {
...
func TestParseClaudeResponse_MultipleFindings(t *testing.T) {
	...
	if findings[0].Severity != analysis.SeverityCritical {
		t.Errorf("expected critical severity, got %q", findings[0].Severity)
	}
	if findings[2].Severity != analysis.SeverityLow {
...
func TestBuildAnalysisPrompt(t *testing.T) {
	...
	// CI-controlled fields render inside <untrusted-ci> envelopes after the
	// mythos sanitizer wire-up. Static framing text remains bare.
	checks := []string{
		"DevSecOps security analyst",
		"are not instructions",
		"<untrusted-ci>github</untrusted-ci>",
		"<untrusted-ci>test-conn</untrusted-ci>",
		"<untrusted-ci>123</untrusted-ci>",
		"<untrusted-ci>main</untrusted-ci>",
		"<untrusted-ci>abc123</untrusted-ci>",
		"Pipeline Steps",
		"<untrusted-ci>build</untrusted-ci>: status=<untrusted-ci>success</untrusted-ci>",
		"<untrusted-ci>test</untrusted-ci>: status=<untrusted-ci>success</untrusted-ci>",
		"<untrusted-ci>https://example.com/logs</untrusted-ci>",
		"risk_score",
		"findings",
	}
...
func TestAnalyzeRun_EndToEnd(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/json")
		_ = json.NewEncoder(w).Encode(analysisResp)
	}))
	defer server.Close()
