...
func TestTestConnection_Success(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		_ = json.NewEncoder(w).Encode(ghUser{Login: "testuser", ID: 1})
	}))
	defer server.Close()
...
func TestListPipelines(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		_ = json.NewEncoder(w).Encode(ghWorkflowsResponse{
			TotalCount: 2,
...
func TestGetPipelineRun(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		_ = json.NewEncoder(w).Encode(ghWorkflowRun{
			ID:         123,
			WorkflowID: 1,
			Status:     "completed",
			Conclusion: "success",
...
func TestListPipelineRuns(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		_ = json.NewEncoder(w).Encode(ghRunsResponse{
			TotalCount: 1,
...
func TestMapGitHubStatus(t *testing.T) {
	tests := []struct {
		...
		{"completed", "skipped", integrations.StatusCancelled},
		{"completed", "neutral", integrations.StatusSuccess},
		{"completed", "timed_out", integrations.StatusTimedOut},
		{"completed", "action_required", integrations.StatusPending},
		{"completed", "stale", integrations.StatusCancelled},
		{"completed", "startup_failure", integrations.StatusFailed},
		{"requested", "", integrations.StatusPending},
		{"action_required", "", integrations.StatusPending},
