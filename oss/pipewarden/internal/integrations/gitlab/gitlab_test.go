...
func TestTestConnection_Success(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		_ = json.NewEncoder(w).Encode(glUser{
			ID:       1,
			Username: "testuser",
			Name:     "Test User",
		})
...
func TestListPipelines(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		_ = json.NewEncoder(w).Encode([]glPipeline{
...
func TestGetPipelineRun(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		_ = json.NewEncoder(w).Encode(glPipelineDetail{
			ID:     42,
			Status: "running",
			Ref:    "main",
			SHA:    "abc123",
...
func TestMapGitLabStatus(t *testing.T) {
	tests := []struct {
		...
		{"manual", integrations.StatusPending},
		{"created", integrations.StatusPending},
		{"scheduled", integrations.StatusPending},
		{"cancelled", integrations.StatusCancelled},
	}
