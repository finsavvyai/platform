...
func TestRealMultipleConnections(t *testing.T) {
	...
	if ghToken != "" {
		_ = manager.Add("github-primary", github.NewClient(github.Config{Token: ghToken}, logger))
	}
	if bbUser != "" && bbPass != "" {
		_ = manager.Add("bitbucket-team", bitbucket.NewClient(bitbucket.Config{
			Username: bbUser, AppPassword: bbPass,
		}, logger))
	}
	if glToken != "" {
		_ = manager.Add("gitlab-cloud", gitlab.NewClient(gitlab.Config{
