...
export async function detectPackageManager(cwd: string = process.cwd()): Promise<DetectionResult> {
  ...
  try {
    ...
    if (packageJson.packageManager) {
      ...
      if ([PACKAGE_MANAGERS.NPM, PACKAGE_MANAGERS.YARN, PACKAGE_MANAGERS.PNPM, PACKAGE_MANAGERS.BUN].includes(pm)) {
...
export function getInstallCommand(pm: PackageManager, packages: string[] = []): string[] {
  ...
  switch (pm) {
    ...
    case PACKAGE_MANAGERS.BUN:
      return [PACKAGE_MANAGERS.BUN, "add", ...packages];
  }
}
...
export function getUpdateCommand(pm: PackageManager, packages: string[] = []): string[] {
  if (packages.length === 0) {
    ...
    switch (pm) {
      ...
      case PACKAGE_MANAGERS.BUN:
        return [PACKAGE_MANAGERS.BUN, "update"];
    }
  }
  ...
  switch (pm) {
    ...
    case PACKAGE_MANAGERS.BUN:
      return [PACKAGE_MANAGERS.BUN, "update", ...packages];
  }
}
...
export function getRemoveCommand(pm: PackageManager, packages: string[]): string[] {
  switch (pm) {
    ...
    case PACKAGE_MANAGERS.BUN:
      return [PACKAGE_MANAGERS.BUN, "remove", ...packages];
  }
}
