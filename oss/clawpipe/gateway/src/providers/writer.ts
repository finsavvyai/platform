/** Writer.com provider adapter — Palmyra models. Supports streaming. */
...
export const writerAdapter: ProviderAdapter = {
  ...
  async call(req: PromptRequest, apiKey: string): Promise<PromptResponse> {
    ...
    const res = await fetch('https://api.writer.com/v1/chat', {
      method: 'POST',
      headers: {
    ...
    if (!res.ok) {
      ...
      throw new Error(`Writer ${res.status}: ${body.slice(0, 200)}`);
    }
