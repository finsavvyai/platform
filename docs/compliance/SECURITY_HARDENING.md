...
- [ ] Public key pinning (HPKP) — **gap**: deferred pending CA strategy.
...
- [ ] Automated rotation for short-lived API tokens — **gap**: manual today; tracked.
...
- [ ] JWT `jti` deny-list for revocation — **gap**: tracked under CC6.2 partial in SOC2_READINESS.
...
- [ ] Zod schemas at every brain route boundary — **partial**: PI1.2 row in SOC2_READINESS.
...
## Next steps before audit (5 items)

1. **Engage SOC 2 auditor** — selection in flight; placeholder in `docs/runbooks/_oncall.md`. Owner: CEO. Target: 2026-06-15.
...
3. **Close JWT `jti` deny-list gap** — implement Redis-backed deny-list and wire into `packages/auth/src/jwt.ts` `verifyToken`. Owner: auth-pkg owner. Target: 2026-07-15.
4. **Zod schemas at every brain route boundary** — close PI1.2 partial. Owner: Brain owner. Target: 2026-07-15.
5. **DR plan exercise** — tabletop the Cloudflare-region-outage scenario end-to-end (per `INCIDENT_RESPONSE.md` quarterly cadence). Owner: Security on-call. Target: 2026-07-31.
