 ...
 * all three parts base64url-encoded. Validates the protected header
 * (alg, typ), the payload claims (iat, exp, nonce, sub), and the
 * signature against the bound public key.
 *
 * Public key may be JWK JSON or PEM-SPKI; the existing
 * `importPublicKey` in ./crypto.ts handles both, so this file stays
 * focused on JWS framing.
 */
