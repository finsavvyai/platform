...
async function makeJws(
  ...
  algOverride?: string,
): Promise<{ jws: string; jwk: JsonWebKey }> {
  ...
  return { jws: `${headerB64}.${payloadB64}.${sigB64}`, jwk: {} as JsonWebKey };
}
...
describe('verifyCompactJws', () => {
  ...
  it('rejects bad signature', async () => {
    ...
    expect(r.ok).toBe(false);
    if (!r.ok) expect(r.reason).toBe('jws_bad_signature');
  });

  it('rejects iat far in the future (jws_future_iat)', async () => {
    const { pair, publicKey } = await setup();
    const future = Math.floor(Date.now() / 1000) + 7200;
    const { jws } = await makeJws(
      { sub: 's', nonce: 'n', iat: future, exp: future + 600 },
      pair.privateKey,
    );
    const r = await verifyCompactJws(jws, { publicKey });
    expect(r).toEqual({ ok: false, reason: 'jws_future_iat' });
  });

  it('rejects when publicKey is unparseable (jws_bad_public_key)', async () => {
    ...
    const r = await verifyCompactJws(jws, { publicKey: 'not-a-jwk-or-pem' });
    expect(r).toEqual({ ok: false, reason: 'jws_bad_public_key' });
  });

  it('rejects when header/payload base64 is corrupt (jws_malformed JSON parse)', async () => {
    const r = await verifyCompactJws('not-base64.not-base64.sig', {
      publicKey: '{"kty":"EC"}',
    });
    expect(r).toEqual({ ok: false, reason: 'jws_malformed' });
  });

  it('pins the malformed-compact-form reason (3-part check)', async () => {
    const { publicKey } = await setup();
    const r = await verifyCompactJws('only-two.parts', { publicKey });
    expect(r).toEqual({ ok: false, reason: 'jws_malformed' });
  });
});
