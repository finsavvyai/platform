...
vi.spyOn(global.Date, 'now').mockImplementation(mockDateNow);
...
describe('RecordingStudio', () => {
  ...
  describe('Initial Render', () => {
    it('should render recording studio components', () => {
      ...
      expect(screen.getByText('🎬 Questro Studio')).toBeInTheDocument();
      expect(screen.getByText('Capture user interactions and generate automated tests with Questro\'s AI-powered recording')).toBeInTheDocument();
    ...
    it('should stop recording when stop button is clicked', async () => {
      ...
      await waitFor(() => {
        expect(mockFetch).toHaveBeenCalledWith('/api/recording/stop', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ sessionId: '1234567890123' })
        });
      });
  ...
  describe('Error Handling', () => {
    ...
    it('should show error toast when recording stop fails', async () => {
      ...
      mockFetch.mockResolvedValueOnce({
        ...
        json: async () => ({
          success: true,
          session: { id: '1234567890123', status: 'recording' }
        })
      } as Response);
  ...
  describe('Session Statistics', () => {
    it('should display session stats when recording is active', async () => {
      ...
      await waitFor(() => {
        ...
        // Verify actions count shows 2 - look for the parent div that contains both the count and label
        const actionsSection = screen.getByText('Actions').closest('div.bg-blue-50');
        expect(actionsSection).toHaveTextContent('2');
      });
    });
  });
  ...
  describe('Action Timeline', () => {
    it('should display recorded actions timeline', async () => {
      ...
      // Check that the actions are displayed in the timeline
      await waitFor(() => {
        // Look for the Recorded Actions section and verify it contains the expected actions
        const timelineSection = screen.getByText('Recorded Actions').closest('div');
        expect(timelineSection).toHaveTextContent('tap');
        expect(timelineSection).toHaveTextContent('type');
        expect(timelineSection).toHaveTextContent('Login Button');
        expect(timelineSection).toHaveTextContent('username@example.com');
      }, { timeout: 3000 });
    });
  });
});