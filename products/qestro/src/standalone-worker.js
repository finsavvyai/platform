/**
 * Standalone Questro Worker
 *
 * Minimal Cloudflare Worker for initial deployment
 * No external dependencies - pure Workers APIs
 */
...
export default {
  async fetch(request, env, ctx) {
    ...
    if (url.pathname === "/api/debug/tables") {
      if (!env.DB) {
        return Response.json({ error: "Database not configured" }, 404);
      }
    ...
    if (url.pathname.startsWith("/api/files/")) {
      ...
      try {
        ...
        if (!r2Bucket) {
          return Response.json({ error: "Bucket not configured" }, 404, {
            headers: corsHeaders,
          });
        }
        ...
        return Response.json(
          { error: "File download failed", details: error.message },
          500,
          { headers: corsHeaders },
        );
      }
