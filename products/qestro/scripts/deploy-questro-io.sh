...
# Questro.io Marketing Site Deployment Script
# Enterprise-focused marketing website deployment

set -e

echo "🏢 Deploying Questro.io Marketing Site"
echo "====================================="
...
# Configuration
SITE_NAME="questro-io-marketing"
BUILD_DIR="questro-io"
DOMAIN="questro.io"
...
preflight_checks() {
    ...
    if [[ ! -f "package.json" ]] || [[ ! -d "questro-io" ]]; then
...
deploy_to_render() {
    ...
    # Use the dedicated render configuration for questro.io
    log "Using render-questro-io.yaml for deployment..."
    if [[ ! -f "render-questro-io.yaml" ]]; then
        error "render-questro-io.yaml not found. Please ensure it exists in the project root."
        exit 1
    fi
    
    # Commit and push changes
    git add .
    git commit -m "Deploy questro.io marketing site

- Built production-ready marketing site
- Enterprise-focused messaging and CTAs
- Optimized for marketing conversions
- Custom domain configuration for questro.io

🚀 Marketing site deployment" || true
    ...
    log "  3. Add: questro.io and www.questro.io"
}
...
deploy_to_aws() {
    ...
    # Configuration
    BUCKET_NAME="questro-io-marketing-site"
    REGION="us-east-1"
...
generate_report() {
    ...
    cat > questro-io-deployment-report.md << EOF
# Questro.io Marketing Site Deployment Report

**Deployment Date:** $(date)
**Site Type:** Marketing & Enterprise
**Domain:** questro.io
...
✅ **Assets Generated:** $(find questro-io/dist -type f | wc -l) files
✅ **Build Size:** $(du -sh questro-io/dist | cut -f1)
...
Add these records in Namecheap for questro.io:
...
- Set up Google Analytics for marketing tracking
- Configure conversion goals for demo requests
- Track enterprise-focused metrics
...
EOF
    ...
    success "Deployment report generated: questro-io-deployment-report.md"
}

# Main deployment menu
main() {
    echo ""
    echo "Select deployment target for Questro.io:"
...
# Run the deployment
log "Starting Questro.io marketing site deployment..."
preflight_checks
build_site
main
generate_report

success "🎉 Questro.io deployment completed!"
