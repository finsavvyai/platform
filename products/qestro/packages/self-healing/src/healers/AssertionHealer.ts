...
export class AssertionHealer implements Healer {
  heal(testResult: TestResult, _testCode?: string): HealingSuggestion[] {
    ...
    testResult.assertions.forEach((assertion, idx) => {
      ...
      if (typeof assertion.actual === 'string') {
        ...
        suggestions.push({
          ...
          confidence: 0.65,
          rationale:
            'Use flexible matching (contains) instead of exact match to handle dynamic content',
          beforeAfterDiff:
