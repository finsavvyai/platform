...
export class APIHealer implements Healer {
  heal(_testResult: TestResult, _testCode?: string): HealingSuggestion[] {
    return [
      ...
      },
      {
        ...
        confidence: 0.72,
        rationale:
          'API error response format changed from singular to array. Update error handling.',
      ...
      },
      {
        ...
        confidence: 0.58,
        rationale:
          'API field may be missing or moved. Add null safety checks and fallback paths.',
        beforeAfterDiff:
