...
export default function DashboardPage() {
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      try {
        // Fetch real test case data from API
        const res = await fetch('/api/test-cases?limit=999');
        const data = await res.json();
        const cases = data.success ? (data.data || []) : [];
        const total = cases.length;
        const active = cases.filter((tc: { status: string }) => tc.status === 'Active').length;
        const draft = cases.filter((tc: { status: string }) => tc.status === 'Draft').length;
        const aiCases = cases.filter((tc: { type: string }) => tc.type === 'ai' || tc.type === 'AI').length;

        setStats({
          totalTests: total,
          successfulTests: active,
          failedTests: draft,
          testCoverage: total > 0 ? Math.round((active / total) * 100) : 0,
          activeDevices: 1,
          aiGeneratedTests: aiCases,
          scheduledTests: 0,
          performanceScore: total > 0 ? 94 : 0,
        });

        // Build recent activity from the most recent test cases
        const recent = cases.slice(0, 4).map((tc: { id: string; title?: string; name?: string; status: string; type: string; createdAt: string }, i: number) => ({
          id: tc.id,
          type: i === 0 ? 'test_run' as const : i === 1 ? 'ai_generation' as const : 'scheduled_test' as const,
          title: tc.title || tc.name || 'Test Case',
          status: tc.status === 'Active' ? 'success' as const : 'scheduled' as const,
          timestamp: new Date(tc.createdAt || Date.now()),
          platform: (tc.type === 'web' ? 'web' : tc.type === 'api' ? 'api' : 'web') as 'web' | 'mobile' | 'api',
        }));
            ...
            <div className="bg-white shadow rounded-lg">
              ...
              <div className="px-6 py-5 space-y-4">
                {user?.subscription && (
                  <>
                    {(() => {
                      const aiCallsRemaining = user.subscription.aiCallsRemaining ?? 0;
                      const webRecordingsRemaining = user.subscription.webRecordingsRemaining ?? 0;
                      const mobileRecordingsRemaining = user.subscription.mobileRecordingsRemaining ?? 0;
                    ...
                    <div>
                      ...
                      <div className="mt-1 w-full bg-gray-200 rounded-full h-2">
                        <div
                          ...
                          style={{
                            width: `${Math.max(0, (aiCallsRemaining / 100) * 100)}%`,
                          }}
                        ></div>
                      </div>
                    </div>
                    ...
                    <div>
                      <div className="flex items-center justify-between text-sm">
                        ...
                        <span className="font-medium">
                          {webRecordingsRemaining} remaining
                        </span>
                      </div>
                      <div className="mt-1 w-full bg-gray-200 rounded-full h-2">
                        <div
                          ...
                          style={{
                            width: `${Math.max(0, (webRecordingsRemaining / 10) * 100)}%`,
                          }}
                        ></div>
                      </div>
                    </div>
                    ...
                    <div>
                      <div className="flex items-center justify-between text-sm">
                        ...
                        <span className="font-medium">
                          {mobileRecordingsRemaining} remaining
                        </span>
                      </div>
                      <div className="mt-1 w-full bg-gray-200 rounded-full h-2">
                        <div
                          ...
                          style={{
                            width: `${Math.max(0, (mobileRecordingsRemaining / 5) * 100)}%`,
                          }}
                        ></div>
                      </div>
                    </div>
