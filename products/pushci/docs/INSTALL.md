...
Install globally for instant startup on every run:
...
Or use npx for one-off usage (slower — downloads each time):

```bash
npx -y pushci init
```

**Don't have npm?** Install Node.js to get npm:

- **macOS**: `brew install node`
- **Linux**: `curl -fsSL https://deb.nodesource.com/setup_lts.x | sudo -E bash - && sudo apt-get install -y nodejs`
- **Windows**: Download from https://nodejs.org

After installing, verify with `npm --version`.
