...
export async function handleWebhook(
  ...
  await env.DB.prepare("UPDATE projects SET build_number = build_number + 1 WHERE repo = ?").bind(event.repo).run();
  await queueCiRun(env, project, run, { trigger: event.event_type });
  await env.RUNNERS.put(`run:${runId}:event`, JSON.stringify(event), { expirationTtl: 3600 });
  return { event, runId };
}
