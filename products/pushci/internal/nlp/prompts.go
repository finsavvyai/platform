...
func ToolDefs() []map[string]interface{} {
	return []map[string]interface{}{
		...
		tool("optimize_pipeline", "Analyze pipeline for performance and cost optimizations", nil),
		tool("fix_pipeline", "Auto-fix a broken pipeline configuration", nil),
		tool("generate_pipeline", "Generate a new CI/CD pipeline from repo analysis", nil),
		tool("heal_pipeline", "Self-heal failed pipeline with auto-retry and fixes", nil),
	}
}
