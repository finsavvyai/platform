...
// intExt2Checks returns build/test/lint for OCaml, Nim, Crystal, Erlang, Vlang.
func intExt2Checks(stacks map[detect.Stack]bool, projects []detect.Project) (build, test, lint []config.Check) {
	...
	if stacks[detect.Erlang] {
		eb, et, el := intErlangChecks(projects)
...
// intErlangChecks returns rebar3 or make checks for Erlang.
func intErlangChecks(projects []detect.Project) ([]config.Check, []config.Check, []config.Check) {
