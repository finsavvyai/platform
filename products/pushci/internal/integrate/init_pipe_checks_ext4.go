...
// intExt4Checks returns build/test/lint for Terraform, Helm, Solidity, Bun, Fortran.
func intExt4Checks(stacks map[detect.Stack]bool, projects []detect.Project) (build, test, lint []config.Check) {
	...
	if stacks[detect.Solidity] {
		sb, st, sl := intSolidityChecks(projects)
...
// intSolidityChecks returns Foundry or Hardhat checks.
func intSolidityChecks(projects []detect.Project) ([]config.Check, []config.Check, []config.Check) {
