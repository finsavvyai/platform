...
func HandleToolCall(params ToolCallParams) ToolCallResult {
	...
	case "pushci_scan":
		return handleScan(params.Arguments)
	case "pushci_recommend":
		return handleRecommend(params.Arguments)
	case "pushci_heal":
		return handleHeal(params.Arguments)
	case "pushci_promote":
		return handlePromote(params.Arguments)
	default:
...
func handleInit(args map[string]any) ToolCallResult {
	...
	for i, p := range projects {
		list[i] = map[string]string{
			"stack": string(p.Stack), "dir": p.Dir, "framework": p.Framework,
		}
	}
	return jsonResult(map[string]any{"projects": list, "config_path": "pushci.yml"})
}
...
func handleRun(args map[string]any) ToolCallResult {
	...
	return jsonResult(map[string]any{
		"passed": run.Passed, "results": results, "duration": run.Elapsed.String(),
	})
}
