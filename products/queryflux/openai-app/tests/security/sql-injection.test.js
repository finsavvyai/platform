...
describe('SQL Injection Security Tests', () => {
    ...
    describe('Query Validation Security', () => {
        it('should detect dangerous SQL patterns', async () => {
            ...
            QueryValidator.mockImplementation(() => ({
                validateSQL: jest.fn().mockImplementation((sql) => {
                    const hasDangerousPattern = dangerousQueries.some(dangerous => sql.toLowerCase().includes(dangerous.toLowerCase()));
        ...
        it('should prevent data leakage through information schema', async () => {
            ...
            QueryValidator.mockImplementation(() => ({
                validateSQL: jest.fn().mockImplementation((sql) => {
                    const hasDataLeak = dataLeakQueries.some(leak => sql.toLowerCase().includes(leak.toLowerCase()));
                    return Promise.resolve({
                        valid: !hasDataLeak,
        ...
        it('should prevent privilege escalation attempts', async () => {
            ...
            QueryValidator.mockImplementation(() => ({
                validateSQL: jest.fn().mockImplementation((sql) => {
                    const hasPrivilegeEscalation = privilegeEscalationQueries.some(priv => sql.toLowerCase().includes(priv.toLowerCase()));
                    return Promise.resolve({
    ...
    describe('Access Control and Authorization', () => {
        it('should enforce query result limits', async () => {
            ...
            const result = await connectionManager.executeQuery(connection.id, 'SELECT * FROM large_table', [], { limit: 1000 });
        ...
        it('should prevent access to restricted database objects', async () => {
            ...
            QueryValidator.mockImplementation(() => ({
                validateSQL: jest.fn().mockImplementation((sql) => {
                    const hasRestrictedAccess = restrictedQueries.some(restricted => sql.toLowerCase().includes(restricted.toLowerCase()));
                    return Promise.resolve({
