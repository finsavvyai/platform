...
import { databaseConnectionManager, DatabaseAdapterFactory } from "./index";
...
export class ConnectionManager {
  ...
  async createConnection(connection: Connection): Promise<ManagedConnection> {
    ...
    if (this.connections.size >= this.maxConnections) {
      throw new Error(
        `Maximum connection limit (${this.maxConnections}) reached`,
      );
    }
    ...
    if (!connection.isActive) {
      ...
      try {
        ...
        connection.lastError =
          error instanceof Error ? error.message : "Unknown error";
        throw new Error(
          `Connection ${connectionId} is not active and reconnection failed: ${connection.lastError}`,
        );
      }
    }
  ...
  getStats(): {
    ...
    return {
      ...
      totalQueries: connections.reduce(
  ...
  private createAdapter(connection: Connection): DatabaseAdapter {
    ...
    // Map database types from Supabase format to our adapter format
    ...
    const adapterType = typeMap[connection.database_type.toLowerCase()];
    if (!adapterType) {
    ...
    return DatabaseAdapterFactory.create({
      ...
      port: config.port || this.getDefaultPort(adapterType),
