import { Database, Server, Leaf, Box, HardDrive, Layers, Activity, Clock, Bug, Zap, Cloud, Network } from 'lucide-react';
import { DatabaseType, DATABASE_CONFIGS, DATABASE_CATEGORIES, DatabaseCategory } from '../types/database';
...
export function DatabaseTypeSelector({ onSelect }: DatabaseTypeSelectorProps) {
  ...
  const renderDatabaseCard = (config: typeof DATABASE_CONFIGS[DatabaseType]) => {
    ...
    return (
      ...
      >
        <div
          ...
          style={{ backgroundColor: config.color + '20', boxShadow: `0 0 20px ${config.color}40` }}
        >
          <Icon className="w-7 h-7" style={{ color: config.color }} />
        </div>
        <h3 className="font-semibold text-center text-sm" style={{ color: theme.colors.text }}>
          {config.name}
        </h3>
  ...
  const categoryOrder: DatabaseCategory[] = ['rdbms', 'cloud', 'nosql', 'aws', 'timeseries', 'cache', 'graph'];
        ...
        return (
          <div key={category}>
            <div className="flex items-center gap-2 mb-3 px-2">
              ...
              <h3 className="text-sm font-semibold uppercase tracking-wide" style={{ color: categoryInfo.color }}>
                {categoryInfo.name}
              </h3>
