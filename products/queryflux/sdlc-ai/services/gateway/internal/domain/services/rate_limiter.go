...
func (r *RateLimiter) CheckRateLimit(ctx context.Context, req *RateLimitRequest) (*RateLimitResult, error) {
	...
	// Run abuse detection asynchronously
	go func() {
		if analysis, err := r.abuseDetector.AnalyzeRequest(ctx, req); err == nil {
			if analysis.BlockSuggested && analysis.Score > 0.8 {
				r.ipBlocker.BlockIP(context.Background(), req.IPAddress, analysis.BlockDuration, analysis.Recommendations[0], "automatic")
			}
...
func (r *RateLimiter) evaluateAgainstCachedInfo(req *RateLimitRequest, info *RateLimitInfo, policy *RateLimitPolicy) *RateLimitResult {
	...
	for _, limit := range policy.Limits {
		...
		if limitStatus.Used >= limit.Value {
			...
			result.Remaining = int(max(0, limit.Value-limitStatus.Used))
...
var (
	rateLimitRequestsTotal = promauto.NewCounterVec(
		...
		[]string{"tenant_id", "endpoint", "allowed"},
	)
	...
	rateLimitCacheHits = promauto.NewCounter(
		prometheus.CounterOpts{
	...
	rateLimitCacheMisses = promauto.NewCounter(
		prometheus.CounterOpts{
	...
	abuseDetectionsTotal = promauto.NewCounterVec(
		prometheus.CounterOpts{
	...
	ipBlocksTotal = promauto.NewCounterVec(
		prometheus.CounterOpts{
			Name: "ip_blocks_total",
			Help: "Total number of IP blocks",
		},
		[]string{"block_type", "reason"},
	)
)
