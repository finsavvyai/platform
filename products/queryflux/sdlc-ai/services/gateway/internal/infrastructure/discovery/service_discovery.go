...
func (sr *ServiceRegistry) GetInstance(ctx context.Context, serviceName string, key string) (*ServiceInstance, error) {
	ctx, span := sr.tracer.Start(ctx, "service_registry.get_instance",
		trace.WithAttributes(
			trace.String("service.name", serviceName),
			trace.String("load_balance.key", key),
		))
	defer span.End()
...
func (hc *HealthChecker) performHealthCheck(ctx context.Context, serviceName, instanceID string) {
	...
	if err != nil {
		hc.registry.UpdateInstanceHealth(ctx, serviceName, instanceID, "unhealthy", fmt.Sprintf("Failed to create request: %v", err))
		return
	}

	resp, err := hc.client.Do(req)
	if err != nil {
		hc.registry.UpdateInstanceHealth(ctx, serviceName, instanceID, "unhealthy", fmt.Sprintf("Health check failed: %v", err))
		return
	}
	defer resp.Body.Close()

	if resp.StatusCode == http.StatusOK {
		hc.registry.UpdateInstanceHealth(ctx, serviceName, instanceID, "healthy", "Health check passed")
	} else {
		hc.registry.UpdateInstanceHealth(ctx, serviceName, instanceID, "unhealthy", fmt.Sprintf("Health check failed with status %d", resp.StatusCode))
	}
}
