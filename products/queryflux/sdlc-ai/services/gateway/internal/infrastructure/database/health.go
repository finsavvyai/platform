...
func (hc *HealthChecker) checkConnectivity(ctx context.Context) CheckResult {
	start := time.Now()
...
func (hc *HealthChecker) checkQueryPerformance(ctx context.Context) CheckResult {
	start := time.Now()
...
func (hc *HealthChecker) checkConnectionPool(ctx context.Context) CheckResult {
	start := time.Now()
	...
	result.Metadata = map[string]interface{}{
		...
		"canceled_acquire_count": stats.CanceledAcquireConns(),
...
func (hc *HealthChecker) checkTableHealth(ctx context.Context) CheckResult {
	start := time.Now()
...
func (hc *HealthChecker) checkReplicationLag(ctx context.Context) CheckResult {
	start := time.Now()
...
func (hc *HealthChecker) checkIndexHealth(ctx context.Context) CheckResult {
	start := time.Now()
...
func (hc *HealthChecker) getDatabaseStats(ctx context.Context) map[string]interface{} {
	...
	stats["connection_pool"] = map[string]interface{}{
		...
		"canceled_acquire_count": poolStats.CanceledAcquireConns(),
	}
...
func (hc *HealthChecker) allChecksHealthy(checks map[string]CheckResult) bool {
...
func (hc *HealthChecker) someChecksHealthy(checks map[string]CheckResult) bool {
	for _, check := range checks {
		if check.Status == "healthy" {
			return true
		}
...
func (hc *HealthChecker) countHealthyChecks(checks map[string]CheckResult) int {
	count := 0
	for _, check := range checks {
		if check.Status == "healthy" {
			count++
...
func (hc *HealthChecker) StartHealthCheckMonitoring(ctx context.Context, interval time.Duration, healthChan chan<- HealthStatus) {
