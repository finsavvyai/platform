...
func (r *UnitTestRunner) runSingleTest(t *testing.T, suite UnitTestSuite, test UnitTest, ctx interface{}) TestResult {
	...
	if test.Timeout > 0 {
		...
		testCtx, cancel = context.WithTimeout(context.Background(), test.Timeout)
		defer cancel()
	}
...
type Fuzzer interface {
	Fuzz(target interface{}) FuzzResult
	Minimize(target interface{}) FuzzResult
}
...
func (f *FuzzTester) fuzzTarget(ctx context.Context, t *testing.T, target FuzzTarget) TestSuiteResult {
	...
	for i := 0; i < f.config.MaxIterations; i++ {
		...
		if f.config.Timeout > 0 {
			var cancel context.CancelFunc
			testCtx, cancel = context.WithTimeout(ctx, f.config.Timeout)
			defer cancel()
		}
...
func createMockUser(id string, username, email, role string) *MockUser {
	return &MockUser{
		ID:       id,
		Username: username,
		Email:    email,
...
func (h *TestHelper) AssertJSON(expected, actual interface{}) {
	...
	assert.JSONEq(h.t, expectedJSON, actualJSON)
}
