...
// AnomalyDetector detects anomalous patterns that might indicate hidden PII
...
func (d *AnomalyDetector) initializeAnomalyIndicators() {
...
func (d *AnomalyDetector) DetectAnomalies(text string) []PIIFinding {
...
func (d *AnomalyDetector) calculateAnomalyConfidence(anomalyType, value string) float64 {
...
func (s *DLPMLService) calculateTextComplexity(text string) float64 {
	...
	specialCharRatio := float64(regexp.MustCompile(`[^a-zA-Z0-9\s]`).FindAllStringIndex(text, -1)) / float64(len(text))
	complexity += specialCharRatio * 0.2

	// Factor in numbers
	numberRatio := float64(regexp.MustCompile(`\d`).FindAllStringIndex(text, -1)) / float64(len(text))
