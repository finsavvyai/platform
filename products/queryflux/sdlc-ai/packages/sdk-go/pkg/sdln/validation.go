...
func (v *DefaultValidator) ValidateStruct(value interface{}) ValidationErrors {
	...
	if val.Kind() != reflect.Struct {
		errors.Add("root", "value must be a struct")
		return errors
	}
...
func NewAPIKeyValidator() *DefaultValidator {
	...
	validator.AddRule("pattern", NewRegexRule("^[a-zA-Z0-9_-]{20,100}$", "API key must contain only alphanumeric characters, hyphens, and underscores"))
...
func NewPasswordValidator() *DefaultValidator {
	...
	validator.AddRule("uppercase", NewRegexRule("[A-Z]", "password must contain at least one uppercase letter"))
	validator.AddRule("lowercase", NewRegexRule("[a-z]", "password must contain at least one lowercase letter"))
	validator.AddRule("digit", NewRegexRule("[0-9]", "password must contain at least one digit"))
	validator.AddRule("special_char", NewRegexRule("[!@#$%^&*()_+=\\[\\]{};':\"\\|,.<>?/~`]", "password must contain at least one special character"))
...
func NewIDValidator() *DefaultValidator {
	...
	// ID validation rules
	validator.AddRule("format", NewRegexRule("^[a-zA-Z0-9_-]+$", "ID must contain only alphanumeric characters, hyphens, and underscores"))
