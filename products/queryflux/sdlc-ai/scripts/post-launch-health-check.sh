...
API_BASE_URL="${API_BASE_URL:-https://api.sdlc.ai}"
...
check_authentication() {
    ...
    duration=$(measure_time curl -s -X POST \
        ...
        -H "Content-Type: application/json" \
        -d '{"email": "healthcheck@sdlc.ai", "password": "'${HEALTH_CHECK_PASSWORD}'"}')
    ...
    "attachments": [
        {
            ...
            "title_link": "https://dashboard.sdlc.ai/health",
            "text": "$message",
            "fields": [
                {
                    "title": "Environment",
                ...
                {
                    "title": "Report",
                    "value": "<https://dashboard.sdlc.ai/reports/$(basename "$REPORT_FILE")|View Details>",
                    "short": false
                }
            ],
