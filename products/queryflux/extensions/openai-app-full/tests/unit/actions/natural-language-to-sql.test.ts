...
describe("NaturalLanguageToSQLEngine", () => {
  ...
  describe("Basic Conversion", () => {
    it("should convert simple natural language to SQL", async () => {
      ...
      const mockResponse = testUtils.createMockOpenAIResponse(
        "SELECT * FROM users",
      );
    ...
    it("should handle complex natural language queries", async () => {
      const request = {
        naturalLanguage:
          "Show me all active users who joined in the last 30 days, ordered by their registration date",
        connectionId: "test-connection",
        databaseType: "postgresql",
      };
    ...
    it("should handle JOIN operations", async () => {
      const request = {
        naturalLanguage:
          "Show me user names and their email addresses from the profiles table",
        connectionId: "test-connection",
        databaseType: "postgresql",
      };
  ...
  describe("Query Validation", () => {
    it("should validate generated SQL syntax", async () => {
      ...
      mockOpenAI.mockResolvedValue(
        testUtils.createMockOpenAIResponse("SELECT * FROM users"),
      );

      // Mock validation failure
      const QueryValidator =
        require("../../../src/security/query-validator.js").QueryValidator;
  ...
  describe("Query Optimization", () => {
    it("should add LIMIT clause when not present", async () => {
      ...
      expect(result.optimization.appliedOptimizations).toContain(
        "Added result limit for safety",
      );
    });
    ...
    it("should suggest missing indexes", async () => {
      ...
      expect(result.success).toBe(true);
      expect(result.optimization.suggestedIndexes).toContain(
        "Consider adding index on email for WHERE clause optimization",
      );
    });
  ...
  describe("Query Analysis and Insights", () => {
    ...
    it("should provide performance considerations", async () => {
      ...
      expect(result.insights.performanceConsiderations.length).toBeGreaterThan(
        0,
      );
    });
  ...
  describe("Error Handling", () => {
    ...
    it("should handle schema analysis failures", async () => {
      ...
      const SchemaAnalyzer =
        require("../../../src/database/schema-analyzer.js").SchemaAnalyzer;
  ...
  describe("Context and Complexity", () => {
    ...
    it("should provide context to OpenAI when available", async () => {
      ...
      mockOpenAI.mockResolvedValue(
        testUtils.createMockOpenAIResponse("SELECT * FROM orders"),
      );
          ...
          messages: expect.arrayContaining([
            expect.objectContaining({
              content: expect.stringContaining(
                "User is looking for customer data from the orders table",
              ),
            }),
          ]),
        }),
