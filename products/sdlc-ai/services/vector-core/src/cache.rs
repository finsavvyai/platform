...
impl CacheService {
    pub async fn new(config: &CacheConfig) -> Result<Self> {
        ...
        let _: Result<String, redis::RedisError> = conn.set("__health_check__", "ok").await;
        let _: Result<String, redis::RedisError> = conn.del("__health_check__").await;
    ...
    fn get_connection(&self) -> impl std::future::Future<Output = Result<redis::aio::Connection>> {
        async {
            self.client
...
impl Cache for CacheService {
        ...
        T: Serialize + Send + Sync,
    {
        ...
        match ttl {
            Some(duration) => {
                let _: () = conn
                    .set_ex(key, data, duration.as_secs() as usize)
                    .await
                    .map_err(|e| AppError::Redis(e))?;
            }
            None => {
...
impl Cache for MemoryCache {
    ...
    where
        T: for<'de> Deserialize<'de> + Send,
    {
        if let Some((data, expires)) = self.cache.get(key) {
            if expires > &std::time::Instant::now() {
                let value: T = bincode::deserialize(&data).map_err(|_| {
                    AppError::Serialization(serde_json::Error::syntax(
    ...
    async fn exists(&self, key: &str) -> Result<bool> {
        if let Some((_, expires)) = self.cache.get(key) {
            Ok(expires > &std::time::Instant::now())
        } else {
            Ok(false)
        }
    }
    ...
    async fn expire(&self, key: &str, ttl: Duration) -> Result<bool> {
        if let Some((data, _)) = self.cache.get(key) {
            let expires = std::time::Instant::now() + ttl;
            self.cache.insert(key.to_string(), (data.clone(), expires));
            Ok(true)
        } else {
            Ok(false)
        }
