...
describe('Auth Store', () => {
  ...
  describe('Login', () => {
    ...
    it('should validate credentials format', async () => {
      ...
      await act(async () => {
        await result.current.login('invalid-email', '123')
      })
  ...
  describe('State Persistence', () => {
    ...
    it('should load state from localStorage on initialization', () => {
      ...
      localStorage.setItem('auth-store', JSON.stringify(mockState))

      const { result } = renderHook(() => useAuthStore())

      expect(result.current.user).toEqual(mockState.state.user)
      expect(result.current.isAuthenticated).toBe(true)
    })
  })
})
