...
type Config struct {
	...
	OpenClaw       OpenClawConfig       `mapstructure:"openclaw" yaml:"openclaw"`
...
// OpenClawConfig holds configuration for the OpenClaw integration
...
func setDefaults() {
	...
	viper.SetDefault("security.cors.allowed_origins", []string{"http://localhost:3000"})
	viper.SetDefault("security.cors.allowed_methods", []string{"GET", "POST", "PUT", "DELETE", "OPTIONS"})
	viper.SetDefault("security.cors.allowed_headers", []string{"Accept", "Authorization", "Content-Type", "X-CSRF-Token", "X-Request-ID", "X-Tenant-ID"})
	...
	viper.SetDefault("security.encryption.key_rotation_interval", "90d")
	...
	viper.SetDefault("cors.allowed_headers", []string{"Accept", "Authorization", "Content-Type", "X-CSRF-Token", "X-Request-ID", "X-Tenant-ID"})
	...
	// OpenClaw defaults
	viper.SetDefault("openclaw.enabled", false)
	viper.SetDefault("openclaw.gateway_url", "http://127.0.0.1:18789")
	viper.SetDefault("openclaw.hook_token", "")
	viper.SetDefault("openclaw.default_channel", "last")
	viper.SetDefault("openclaw.timeout_seconds", 30)
	viper.SetDefault("openclaw.max_retries", 3)
	viper.SetDefault("openclaw.retry_delay_ms", 1000)
}
...
func validate(config *Config) error {
	...
	if config.Cloudflare.AccountID == "" && getEnvironment() == Production {
