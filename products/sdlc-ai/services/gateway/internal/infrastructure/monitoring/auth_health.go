...
func (m *AuthHealthMonitor) checkJWTService(ctx context.Context) ServiceHealthStatus {
	...
	_, err := m.jwtService.GenerateTokenPair(
		ctx,
		// Use actual UUIDs in real implementation
		nil, // userID
		nil, // tenantID
...
func (m *AuthHealthMonitor) updatePrometheusMetrics() {
	...
	m.metrics.healthStatus.WithLabelValues("jwt_service").Set(float64(m.status.JWTService.Status))
	m.metrics.healthStatus.WithLabelValues("blacklist_service").Set(float64(m.status.BlacklistService.Status))
	m.metrics.healthStatus.WithLabelValues("session_manager").Set(float64(m.status.SessionManager.Status))
	m.metrics.healthStatus.WithLabelValues("credential_manager").Set(float64(m.status.CredentialManager.Status))
}
...
func (m *AuthHealthMonitor) getCurrentMetrics() AuthMetrics {
	...
	if m.config.EnableMetrics {
		metrics.TotalTokensGenerated = int64(m.metrics.tokensGenerated.Get())
		metrics.TotalTokensValidated = int64(m.metrics.tokensValidated.Get())
		metrics.TotalTokensRevoked = int64(m.metrics.tokensRevoked.Get())
		metrics.TotalRefreshTokensUsed = int64(m.metrics.refreshTokensUsed.Get())
		metrics.ActiveTokens = int64(m.metrics.activeTokens.Get())
		metrics.ActiveSessions = int64(m.metrics.activeSessions.Get())
		metrics.BlacklistedTokens = int64(m.metrics.blacklistedTokens.Get())

		// Get histogram metrics
		validationLatencyMetric, err := m.metrics.validationLatency.GetMetricWithLabelValues()
		if err == nil {
			metrics.ValidationLatencyMs = validationLatencyMetric.(prometheus.Histogram).GetHistogram().Sum * 1000
		}

		generationLatencyMetric, err := m.metrics.generationLatency.GetMetricWithLabelValues()
		if err == nil {
			metrics.GenerationLatencyMs = generationLatencyMetric.(prometheus.Histogram).GetHistogram().Sum * 1000
		}

		// Get error count
		errorCountMetric, err := m.metrics.errorCount.GetMetricWithLabelValues()
		if err == nil {
			metrics.TotalAuthenticationErrors = int64(errorCountMetric.(prometheus.Counter).Get())
		}
	}
...
func (m *AuthHealthMonitor) GetPrometheusMetrics() (prometheus.Gatherer, error) {
	...
	// Register metrics
	registry.MustRegister(m.metrics.healthStatus)
	registry.MustRegister(m.metrics.responseTime)
	registry.MustRegister(m.metrics.errorCount)
	registry.MustRegister(m.metrics.tokensGenerated)
	registry.MustRegister(m.metrics.tokensValidated)
	registry.MustRegister(m.metrics.tokensRevoked)
	registry.MustRegister(m.metrics.refreshTokensUsed)
	registry.MustRegister(m.metrics.validationLatency)
	registry.MustRegister(m.metrics.generationLatency)
	registry.MustRegister(m.metrics.activeTokens)
	registry.MustRegister(m.metrics.activeSessions)
	registry.MustRegister(m.metrics.blacklistedTokens)
