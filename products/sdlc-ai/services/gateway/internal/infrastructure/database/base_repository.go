...
func (br *BaseRepository) executeCommand(ctx context.Context, query string, args ...interface{}) (pgx.CommandTag, error) {
	br.logger.WithFields(logrus.Fields{
		"query": query,
		"args":  args,
...
func (br *BaseRepository) count(ctx context.Context, tableName string, conditions string, args ...interface{}) (int, error) {
	query := fmt.Sprintf("SELECT COUNT(*) FROM %s %s", pgx.Identifier{tableName}.Sanitize(), conditions)
...
func (br *BaseRepository) exists(ctx context.Context, tableName string, conditions string, args ...interface{}) (bool, error) {
	query := fmt.Sprintf("SELECT EXISTS(SELECT 1 FROM %s %s)", pgx.Identifier{tableName}.Sanitize(), conditions)
