...
func TestPostmanGenerator_Generate(t *testing.T) {
	...
	config := PostmanConfig{
		...
		Schema:        "https://schema.getpostman.com/json/collection/v2.1.0/collection.json",
		DefaultServer: "https://api.sdlc.ai/v1",
		Servers: []string{
			"https://api.sdlc.ai/v1",
			"https://staging-api.sdlc.ai/v1",
		},
		AuthType: "bearer",
		Environment: map[string]string{
			"baseUrl": "https://api.sdlc.ai/v1",
			"token":   "{{authToken}}",
		},
		GenerateTests:        true,
		IncludeDocumentation: true,
...
func TestPostmanGenerator_GenerateVariables(t *testing.T) {
	...
	for _, v := range variables {
		if v.Key == "baseUrl" {
			assert.Equal(t, "https://api.sdlc.ai/v1", v.Value)
