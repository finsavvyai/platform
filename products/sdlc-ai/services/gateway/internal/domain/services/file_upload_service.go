...
type UploadRequest struct {
	...
	RetentionPolicy *models.JSONB `json:"retention_policy"`
}
...
func (s *FileUploadService) UploadFile(ctx context.Context, req *UploadRequest) (*UploadResponse, error) {
	ctx, span := otel.Tracer("file-upload-service").Start(ctx, "UploadFile")
	defer span.End()
	...
	// Step 7: Store file in R2
	storagePath, err := s.storageProvider.Store(ctx, storage.StoreRequest{
		TenantID:    req.TenantID,
		DocumentID:  uuid.New(), // Will be generated when creating document
		Filename:    filename,
	...
	if err != nil {
		s.logger.WithError(err).Error("File storage failed")
		return nil, fmt.Errorf("file storage failed: %w", err)
	}

	// Step 8: Create document record
	document := s.createDocumentRecord(req, filename, contentType, checksum, storagePath, encryptionKeyID, metadata)
...
func (s *FileUploadService) extractFileMetadata(ctx context.Context, content []byte, contentType string) (map[string]interface{}, error) {
	ctx, span := otel.Tracer("file-upload-service").Start(ctx, "ExtractFileMetadata")
	defer span.End()
...
func (s *FileUploadService) encryptContent(ctx context.Context, tenantID uuid.UUID, content []byte) ([]byte, string, error) {
	ctx, span := otel.Tracer("file-upload-service").Start(ctx, "EncryptContent")
	defer span.End()
...
func (s *FileUploadService) createDocumentRecord(
	...
	document.StorageBucket = s.storageProvider.GetBucketName()
	...
	if req.RetentionPolicy != nil {
		document.RetentionPolicy = *req.RetentionPolicy
	}
