...
func NewAuditService(client *Client, storage AuditLogStorage) *AuditService {
	service := &AuditService{
		...
		consentMgr:   NewConsentManager(storage),
	}
...
func (s *AuditService) GenerateComplianceReport(ctx context.Context, req *ComplianceReportRequest) (*ComplianceReport, error) {
	...
	switch req.Type {
	case "gdpr":
		return s.generateGDPRReport(ctx, req, result.Logs)
	case "hipaa":
		return s.generateHIPAAReport(ctx, req, result.Logs)
	case "sox":
		return s.generateSOXReport(ctx, req, result.Logs)
	default:
		return s.generateCustomComplianceReport(ctx, req, result.Logs)
	}
}
...
func (s *AuditService) GetComplianceStatus(ctx context.Context, tenantID string) (*ComplianceStatus, error) {
	...
	query := &AuditQuery{
		TenantID:  tenantID,
		StartTime: NewTimestamp(time.Now().UTC().AddDate(0, 0, -30)), // Last 30 days
		Limit:     5000,
	}
