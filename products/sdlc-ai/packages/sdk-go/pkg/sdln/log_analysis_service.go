...
// Histogram represents histogram
...
func NewLogAggregator() *LogAggregator {
	return &LogAggregator{
		...
		timeWindows:  []time.Duration{time.Minute, time.Hour, time.Day},
...
func (las *LogAnalysisService) GetPatterns(ctx context.Context, tenantID string, timeRange *TimeRange) ([]LogPattern, error) {
	...
	for _, pattern := range las.patterns.patterns {
		...
		if timeRange != nil {
			if pattern.FirstSeen.Time.Before(timeRange.From.Time) || pattern.LastSeen.Time.After(timeRange.To.Time) {
				continue
			}
		}
...
func (las *LogAnalysisService) DetectAnomalies(ctx context.Context, tenantID string, timeRange *TimeRange) ([]PatternAnomaly, error) {
	...
	for _, pattern := range las.patterns.patterns {
		...
		timeSinceFirst := time.Since(pattern.FirstSeen.Time)
...
type LogSearchResult struct {
	...
	Tota  int64                  `json:"total"`
	Query *Adva    ncedLogQuery  `json:"query"`
	Took  time.Duration          `json:"took"`
	Aggs  map[string]in    terface{} `json:"aggs,omitempty"`
}
...
func (las *LogAnalysisService) GetLogMetrics(ctx context.Context, tenantID string, timeRange *TimeRange) (*LogMetricsReport, error) {
	...
	report.Histograms["latency"] = &Histogram{
...
type LogMetricsReport struct {
	...
	TimeRange  TimeRa                `json:"time_range"`
	Timestamp  Timestamp             `json:"timestamp"`
	Counts     map[strinnt64         `json:"counts"`
	Rates      map[string]float      `json:"rates"`
	Histograms map[string]*Histog    `json:"histograms"`
	TopLogs    []string              `json:"top_logs"`
	Errors     []ErrorPern           `json:"errors"`
}
