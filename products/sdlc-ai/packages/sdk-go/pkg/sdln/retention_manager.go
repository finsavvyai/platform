	...
	schedules := []RetentionSchedule{
		{
			...
			Enabled:       true,
			MaxRetries:    3,
			NextRun:       NewTimestamp(time.Now().UTC().AddDate(0, 0, 1)), // Tomorrow at 2 AM
			Priority:      1,
		},
		{
			...
			Enabled:       true,
			MaxRetries:    3,
			NextRun:       NewTimestamp(time.Now().UTC().AddDate(0, 1, 0)), // Next month
			Priority:      2,
		},
		{
			...
			Enabled:       true,
			MaxRetries:    3,
			NextRun:       NewTimestamp(time.Now().UTC().AddDate(3, 0, 0)), // Next quarter
			Priority:      3,
		},
	}
