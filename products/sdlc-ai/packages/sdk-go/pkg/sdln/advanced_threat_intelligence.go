...
func extractFeatures(request *ThreatAnalysisRequest) map[string]float64 {
	...
	if request.UserAgent != "" {
		...
		features["is_bot"] = 0.0
		if contains(request.UserAgent, "bot") || contains(request.UserAgent, "crawler") {
			features["is_bot"] = 1.0
		}
	}
...
func (zdh *ZeroDayHunter) DetectZeroDayThreats(request *ThreatAnalysisRequest) []ZeroDayThreat {
	...
	// Anomaly detection
	anomalies := zdh.anomalyDetector.DetectAnomalies(request)
...
func calculateInjectionProbability(request *ThreatAnalysisRequest) float64 {
	...
	if request.URL != "" && contains(request.URL, "'") || contains(request.URL, "\"") {
		probability += 0.3
	}
