...
func GetStructFields(v interface{}) map[string]interface{} {
	...
	for i := 0; i < val.NumField(); i++ {
		...
		case reflect.Uint, reflect.Uint8, reflect.Uint16, reflect.Uint32, reflect.Uint64:
			fieldValue = fieldValue.Uint()
		case reflect.Float32, reflect.Float64:
			fieldValue = fieldValue.Float()
		case reflect.Bool:
			fieldValue = fieldValue.Bool()
		case reflect.Slice, reflect.Array:
			if fieldValue.Type().Elem().Kind() == reflect.Uint8 {
				// Handle []byte
				fieldValue = fieldValue.Bytes()
			} else {
				// Handle other slices
		...
		case reflect.Struct:
			if fieldValue.Type().String() == "Time" {
				...
				fieldValue = GetStructFields(fieldValue.Interface())
			}
		case reflect.Ptr:
			if !fieldValue.IsNil() {
				fieldValue = GetStructFields(fieldValue.Interface())
...
func UnixToTime(timestamp int64) Timestamp {
	return Time(time.Unix(timestamp, 0).UTC())
}
...
func DurationSince(t Time) time.Duration {
...
func IsTimeout(err error) bool {
	var timeoutErr *TimestampoutError
...
func ToInt(v interface{}) int {
	...
	case float64:
		return int(val)
	case string:
		if i, err := fmt.Sscanf(val, "%d", &i); err == nil {
			return i
		}
		return 0
	case bool:
...
func ToFloat64(v interface{}) float64 {
	...
	case int64:
		return float64(val)
	case string:
		if f, err := fmt.Sscanf(val, "%f", &f); err == nil {
			return f
		}
		return 0.0
	case bool:
