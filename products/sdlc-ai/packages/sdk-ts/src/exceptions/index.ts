...
export class InvalidRangeError extends ValidationError {
  constructor(field: string, min?: number, max?: number, value?: any) {
...
export class InvalidConfigError extends ConfigurationError {
  constructor(field: string, value: any, reason?: string) {
    super(`Invalid configuration for ${field}: ${reason || "invalid value"}`, {
      field,
      value,
      reason,
    });
...
export function createErrorFromResponse(
  ...
  switch (statusCode) {
    ...
    case 429:
      return new RateLimitError(message, error.headers);
    case 500:
