...
from sdlc_sdk.auth import OAuthAuth, JWTAuth
from sdlc_sdk.exceptions import RateLimitError, AuthenticationError, ValidationError
from sdlc_sdk.models.user import BulkUserCreate, UserCreate
from sdlc_sdk.models.rag import RAGQuery, RAGConfig
...
def bulk_operations_example():
    ...
    config = Config(
        base_url="https://api.sdlc.ai", retry=dict(max_retries=5, retry_backoff=2.0)
    )
...
def error_handling_example():
    ...
    config = Config(
        ...
        retry=dict(max_retries=3, retry_backoff=1.0, retry_jitter=True),
    )
