...
BEGIN
    SELECT id INTO system_tenant_uuid FROM tenants WHERE domain = 'system.sdlc.ai';
    ...
    INSERT INTO api_keys (
        ...
        uuid_generate_v4(),
        system_tenant_uuid,
        (SELECT id FROM users WHERE tenant_id = system_tenant_uuid AND email = 'system@sdlc.ai'),
        'System API Key',
...
BEGIN
    SELECT id INTO demo_tenant_uuid FROM tenants WHERE domain = 'demo.sdlc.ai';
    ...
    IF demo_tenant_uuid IS NOT NULL THEN
        ...
        -- Get admin user ID
        SELECT id INTO admin_user_uuid FROM users WHERE tenant_id = demo_tenant_uuid AND email = 'admin@demo.sdlc.ai';
...
BEGIN
    SELECT id INTO demo_tenant_uuid FROM tenants WHERE domain = 'demo.sdlc.ai';
    SELECT id INTO demo_admin_uuid FROM users WHERE tenant_id = demo_tenant_uuid AND email = 'admin@demo.sdlc.ai';
