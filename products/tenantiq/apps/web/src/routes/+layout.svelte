<script lang="ts">
	...
	// Skip on /auth/callback — the callback page exchanges the code, sets
	// the cookie, and owns the post-login auth/tenant store state itself.
	// Probing here races the exchange and can clear the just-set user.
	if (browser) initSentry();
	if (browser && !window.location.pathname.startsWith('/auth/callback')) {
		// Safety timeout: never show spinner forever if API is unreachable.
		// 6s is the upper bound for a healthy /auth/me round-trip; beyond
		// that, treat the user as signed-out and let SignInHero render.
...
{#if $auth.loading && !isPublicRoute}
	<div class="flex h-screen items-center justify-center bg-[var(--color-bg)]" role="status" aria-live="polite">
		<div class="text-center">
			<div class="mx-auto mb-4 h-8 w-8 animate-spin rounded-full border-2 border-[var(--color-primary)] border-t-transparent" aria-hidden="true"></div>
			<p class="text-sm text-[var(--color-text-secondary)]">Loading your console…</p>
		</div>
	</div>
{:else}
