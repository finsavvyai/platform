<script lang="ts">
	...
	import ConnectWiseForm from '$lib/components/integrations/ConnectWiseForm.svelte';
	import ConnectWiseMappings from '$lib/components/integrations/ConnectWiseMappings.svelte';
	...
	interface Mapping {
		...
		cwCompanyId: string | null; cwCompanyName: string | null;
		autoMatched: boolean;
	}
	...
	async function loadData() {
		loading = true;
		try {
			const status = await api.get<SyncStatus>('/integrations/connectwise/status').catch(() => null);
			syncStatus = status;
			if (status && status.status !== 'disconnected') {
				const [m, h] = await Promise.all([
					api.get<{ mappings: Mapping[] }>('/integrations/connectwise/mappings').catch(() => ({ mappings: [] })),
					api.get<{ history: HistoryEntry[] }>('/integrations/connectwise/status').catch(() => ({ history: [] }))
				]);
				mappings = m.mappings;
	...
	async function triggerSync() {
		syncing = true;
		try {
			await api.post('/integrations/connectwise/sync');
	...
	async function disconnect() {
		disconnecting = true;
		try {
			await api.delete('/integrations/connectwise/disconnect');
			toasts.success('ConnectWise disconnected');
			syncStatus = null;
			mappings = [];
			history = [];
		} catch (e) {
...
<svelte:head>
	<title>ConnectWise PSA | TenantIQ</title>
</svelte:head>
...
<div class="space-y-6">
	...
	<div class="flex items-start justify-between">
		<div>
			<h1 class="text-2xl font-bold text-[var(--color-text)]">ConnectWise PSA</h1>
			<p class="text-[var(--color-text-secondary)]">Sync companies, tickets, and configurations.</p>
		</div>
		{#if isConnected}
			<button
	...
	{:else if !isConnected}
		<ConnectWiseForm onConnected={loadData} />
	{:else if syncStatus}
		...
		<ConnectWiseMappings {mappings} onUpdate={loadData} />
...
<ConfirmModal
	open={showDisconnect}
	title="Disconnect ConnectWise"
	description="This will remove the ConnectWise integration and stop all syncing. Company mappings will be preserved for reconnection."
