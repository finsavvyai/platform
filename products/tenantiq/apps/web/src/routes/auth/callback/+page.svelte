<script lang="ts">
	...
	type TenantRow = { id: string; displayName: string; domain: string; status: 'active' | 'suspended' | 'disconnected'; lastSyncAt: string | null };
		...
		if (errMsg) {
			...
			(async () => {
				try {
					...
					const { user } = (await resp.json()) as { user: Record<string, unknown> };
					auth.setUser(user as any);
					// Layout's /auth/me probe fired before the cookie existed and
					// will 401 with auth.clear(). Pre-fetch tenants here so we own
					// the post-login state and avoid a hard reload race.
					try {
						const td = await api.get<{ tenants: TenantRow[] }>('/tenants');
						tenant.setTenants(td.tenants);
					} catch {
						tenant.setLoaded();
					}
					// SPA navigation preserves the auth/tenant stores we just set.
