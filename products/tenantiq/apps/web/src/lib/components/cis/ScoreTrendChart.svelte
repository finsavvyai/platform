...
<div class="trend-card">
	<header class="trend-head">
		<div>
			<h3>Score Trend</h3>
			<p class="trend-sub">Last {summary.windowDays} days · {summary.scanCount} scan{summary.scanCount === 1 ? '' : 's'}</p>
		</div>
	...
	{#if points.length === 0}
		<div class="empty">
			<p>No scan history yet. Run a CIS scan to start the trend.</p>
		</div>
	{:else}
		<div class="chart-wrap">
			<svg viewBox="0 0 {W} {H}" class="chart" role="img" aria-label="CIS overall score trend">
				...
				{#each points as p, i}
					...
					<circle cx={x} cy={y} r="3.5" class="point">
						<title>{fmtDate(p.date)}: {p.score}/100 ({p.passCount} pass / {p.failCount} fail)</title>
					</circle>
				{/each}
			</svg>
		</div>
		...
		<div class="trend-footer">
			<div class="metric">
				<span class="metric-label">Earliest</span>
				<span class="metric-value">{summary.earliestScore ?? '—'}</span>
			</div>
			<div class="metric">
				<span class="metric-label">Latest</span>
				<span class="metric-value">{summary.latestScore ?? '—'}</span>
			</div>
			<div class="metric">
				<span class="metric-label">Δ</span>
				<span class="metric-value" style="color: {directionColor};">
