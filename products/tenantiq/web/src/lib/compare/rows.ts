/**
 * Row arrays per competitor. Sourced from .luna/tenantiq/competitive/<vendor>-analysis.md.
 * Imported by /compare and /compare/[vendor] so both views render the same data.
 */
import type { CompetitorRow } from './vendors';

export const horizontalAiRows: CompetitorRow[] = [
	{ feature: 'Multi-tenant scope (one console, every customer\'s tenant)', detail: 'MSPs manage 9–250+ Azure AD tenants concurrently — horizontal Claude lives inside ONE tenant\'s M365 at a time', tenantiq: { status: 'unique' }, them: { status: 'no' } },
	{ feature: 'Per-tenant CIS benchmark overrides with audit trail', detail: 'Each customer accepts/omits controls independently with justification; ScubaGear-style', tenantiq: { status: 'unique' }, them: { status: 'no' } },
	{ feature: 'Drift attribution to actor (who-did-it across tenants)', detail: 'Cross-references directoryAudits — Claude in Excel sees the file, not the change history of 25 customer tenants', tenantiq: { status: 'unique' }, them: { status: 'no' } },
	{ feature: 'License-tier upsell on remediation block (per-tenant)', detail: '402 LICENSE_UPGRADE_REQUIRED with concrete cost per tenant — needed for MSP cost-pass-through', tenantiq: { status: 'unique' }, them: { status: 'no' } },
	{ feature: 'Cross-tenant rollups (MSP backup health / benchmark / alerts)', detail: 'Single page showing every customer\'s posture; horizontal AI has no concept of "all customers"', tenantiq: { status: 'unique' }, them: { status: 'no' } },
	{ feature: 'Account-deletion cascade across MSP\'s data (GDPR Art. 17)', detail: '33-table cascade with contract test; horizontal AI doesn\'t own customer-facing data lifecycle', tenantiq: { status: 'unique' }, them: { status: 'no' } },
	{ feature: 'MSP-billing integration (per-tenant + Microsoft Marketplace co-sell)', tenantiq: { status: 'yes' }, them: { status: 'no' } },
	{ feature: 'AI explainer for security findings (Claude per-control)', detail: 'TenantIQ uses Claude FOR security analysis; horizontal AI uses Claude for productivity', tenantiq: { status: 'yes' }, them: { status: 'partial', note: 'Same model, different surface' } },
	{ feature: 'Lives in Excel/Word/PowerPoint/Outlook to draft documents', tenantiq: { status: 'no', note: 'Not the goal' }, them: { status: 'yes' } },
];

export const coreviewRows: CompetitorRow[] = [
	{ feature: 'Public per-tenant pricing (no "Request a quote" gate)', detail: 'CoreView gates all 4 tiers behind a sales conversation; TenantIQ ships transparent $45–$99/tenant/mo on the pricing page', tenantiq: { status: 'unique' }, them: { status: 'no', note: 'All tiers are "Request pricing"' } },
	{ feature: 'Per-tenant CIS overrides with audit-grade justification', detail: 'CoreView treats exceptions as documentary process (accepted-risk + owner + review date); TenantIQ stores them as structural per-control rows with attribution', tenantiq: { status: 'unique' }, them: { status: 'partial', note: 'Documentary only' } },
	{ feature: 'Drift attribution to actor (who changed what when)', detail: 'CoreView compares state vs template; the "who" is missing publicly. TenantIQ cross-references directoryAudits for actor attribution', tenantiq: { status: 'unique' }, them: { status: 'no' } },
	{ feature: 'MSP-native model (per-tenant pricing, white-label, MSP RBAC)', detail: 'CoreView\'s MSP page reuses enterprise messaging; logos are Panasonic / Morgan Stanley / Nintendo / Berkshire Hathaway — not MSP partners', tenantiq: { status: 'unique' }, them: { status: 'partial' } },
	{ feature: 'MCP server (own + composer of external MCP servers)', detail: 'TenantIQ is registry-listable; Corey AI is non-autonomous + no developer agent surface', tenantiq: { status: 'unique' }, them: { status: 'no' } },
	{ feature: 'Autonomous agent loop with rollback (auto-fix + 60s anomaly watch)', tenantiq: { status: 'unique' }, them: { status: 'no', note: 'Corey is read-only / per-action approval' } },
	{ feature: 'General CRUD admin API (not just workflow triggers)', detail: 'CoreView API is OAuth + workflow-trigger only; no GraphQL, no third-party UI on top', tenantiq: { status: 'partial', note: 'Tenant-scoped REST + MCP' }, them: { status: 'partial', note: 'Workflow-trigger only' } },
	{ feature: 'Free public no-auth scan (lead-gen funnel)', detail: 'CoreView has free Tenant Security Scanner — comparable funnel', tenantiq: { status: 'yes' }, them: { status: 'yes' } },
	{ feature: 'CIS Microsoft 365 v3.1 evaluation', tenantiq: { status: 'yes', note: '121 controls, 31+ wired to live Graph' }, them: { status: 'yes', note: 'CIS baselines + drift alerts' } },
	{ feature: 'ISO 27001:2022 Annex A telemetry-evaluable controls', detail: 'TenantIQ ships 25 evaluable + honest disclosure of 68 organisational. CoreView is CIS-only', tenantiq: { status: 'yes' }, them: { status: 'no' } },
	{ feature: 'License-tier upsell on remediation block (with concrete cost)', detail: 'CoreView\'s per-user model can\'t cleanly price this per remediation', tenantiq: { status: 'unique' }, them: { status: 'no' } },
	{ feature: '"Tenant resilience" / golden-image restore', detail: 'CoreView\'s anchor message — backed by years of enterprise references. TenantIQ doesn\'t fight here', tenantiq: { status: 'partial', note: 'Snapshot + revert exists; not the pitch' }, them: { status: 'yes' } },
	{ feature: 'GovRAMP Premier Member', tenantiq: { status: 'no' }, them: { status: 'yes' } },
	{ feature: 'Self-serve trial (no demo gate)', tenantiq: { status: 'yes', note: '14-day, no credit card' }, them: { status: 'partial', note: 'Free tools only; full platform demo-gated' } },
];

export const nerdioRows: CompetitorRow[] = [
...
export const syskitRows: CompetitorRow[] = [
...
export const bettercloudRows: CompetitorRow[] = [
...
export const avepointRows: CompetitorRow[] = [
...
export const augmenttRows: CompetitorRow[] = [
...
export const lighthouseRows: CompetitorRow[] = [
...
export const rowsBySlug: Record<string, CompetitorRow[]> = {
	'horizontal-ai': horizontalAiRows,
	coreview: coreviewRows,
	nerdio: nerdioRows,
	syskit: syskitRows,
	bettercloud: bettercloudRows,
	avepoint: avepointRows,
	augmentt: augmenttRows,
