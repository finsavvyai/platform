<script lang="ts">
	...
	import KaseyaForm from '$lib/components/integrations/KaseyaForm.svelte';
	...
	interface Mapping {
		...
		kaseyaAccountId: string | null; kaseyaAccountName: string | null;
		autoMatched: boolean;
	}
	...
	async function loadData() {
		loading = true;
		try {
			const status = await api.get<SyncStatus>('/integrations/kaseya/status').catch(() => null);
			syncStatus = status;
			if (status && status.status !== 'disconnected') {
				const [m, h] = await Promise.all([
					api.get<{ mappings: Mapping[] }>('/integrations/kaseya/mappings').catch(() => ({ mappings: [] })),
					api.get<{ history: HistoryEntry[] }>('/integrations/kaseya/status').catch(() => ({ history: [] }))
				]);
				mappings = m.mappings;
	...
	async function triggerSync() {
		syncing = true;
		try {
			await api.post('/integrations/kaseya/sync');
	...
	async function disconnect() {
		disconnecting = true;
		try {
			await api.delete('/integrations/kaseya/disconnect');
			toasts.success('Kaseya BMS disconnected');
			syncStatus = null;
			mappings = [];
			history = [];
		} catch (e) {
...
<svelte:head>
	<title>Kaseya BMS | TenantIQ</title>
</svelte:head>
...
<div class="space-y-6">
	...
	<div class="flex items-start justify-between">
		<div>
			<h1 class="text-2xl font-bold text-[var(--color-text)]">Kaseya BMS</h1>
			<p class="text-[var(--color-text-secondary)]">Sync accounts, tickets, and billing workflows.</p>
		</div>
		{#if isConnected}
			<button
	...
	{:else if !isConnected}
		<KaseyaForm onConnected={loadData} />
	{:else if syncStatus}
		<div class="rounded-lg border border-[var(--color-border)] bg-[var(--color-surface)] p-4">
			...
			<div class="mt-4 grid grid-cols-2 gap-4 sm:grid-cols-4">
				<div>
					<p class="text-xs text-[var(--color-text-secondary)]">Accounts Synced</p>
...
<ConfirmModal
	open={showDisconnect}
	title="Disconnect Kaseya BMS"
	description="This will remove the Kaseya BMS integration and stop all syncing. Account mappings will be preserved for reconnection."
