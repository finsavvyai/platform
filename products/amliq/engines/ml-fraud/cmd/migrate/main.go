...
func (mgr *MigrationManager) gotoVersion(args []string) {
	...
	if err := mgr.mig.Migrate(uint(version)); err != nil && err != migrate.ErrNoChange {
		log.Fatalf("❌ Migration failed: %v", err)
	}
...
func (mgr *MigrationManager) showStatus() {
	...
	for _, file := range migrations {
		...
		if len(parts) >= 2 {
			...
			status := "🔜 Pending"
			if uint(num) <= version && !dirty {
				status = "✅ Applied"
			} else if uint(num) <= version && dirty {
				status = "⚠️  Partial"
			}
...
func (mgr *MigrationManager) resetDatabase() {
	...
	if err := mgr.mig.Steps(-1000); err != nil {
...
func (mgr *MigrationManager) backupDatabase(args []string) {
	...
	_ = fmt.Sprintf("pg_dump -h %s -p %s -U %s -d %s -f %s --verbose",
		mgr.config.Host,
		mgr.config.Port,
		mgr.config.User,
		mgr.config.Database,
