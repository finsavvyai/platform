...
func TestAPIKeyMiddleware_ValidateAPIKey(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			// Create a fresh router per test case to avoid duplicate route panic
			router := setupGinTest()
			// Create a test route
...
func TestAPIKeyMiddleware_RateLimitByTier(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Create a fresh router per test case
			router := setupGinTest()
			// Create test route
			router.GET("/test-rate-limit", func(c *gin.Context) {
				...
				if middleware.apiKeyService.rateLimitService != nil {
					...
					middleware.apiKeyService.rateLimitService.IncrementCounter(
						c.Request.Context(),
						rateLimitKey,
						60,
					)
