...
const (
	StatusHealthy   ComponentHealthStatus = "healthy"
	StatusUnhealthy ComponentHealthStatus = "unhealthy"
	StatusDegraded  ComponentHealthStatus = "degraded"
	StatusUnknown   ComponentHealthStatus = "unknown"
)
...
// SystemHealthChecker manages health checks for system components
...
// SystemHealthConfig contains configuration for health checking
...
func NewSystemHealthChecker(config SystemHealthConfig, buildInfo BuildInfo) *SystemHealthChecker {
...
func (hc *SystemHealthChecker) addDefaultChecks() {
	// Runtime check
...
func (hc *SystemHealthChecker) AddCheck(check HealthCheck) {
	hc.checks[check.Name] = check
}

// RemoveCheck removes a health check
func (hc *SystemHealthChecker) RemoveCheck(name string) {
	delete(hc.checks, name)
	delete(hc.results, name)
}

// CheckHealth performs all health checks
func (hc *SystemHealthChecker) CheckHealth(ctx context.Context) SystemHealth {
...
func (hc *SystemHealthChecker) runCheck(ctx context.Context, check HealthCheck) ComponentHealth {
	start := time.Now()
	defer func() {
...
func (hc *SystemHealthChecker) statusToGaugeValue(status ComponentHealthStatus) float64 {
	switch status {
	case StatusHealthy:
		return 1.0
	case StatusDegraded:
...
func (hc *SystemHealthChecker) calculateSystemHealth() SystemHealth {
...
func (hc *SystemHealthChecker) calculateSummary(components map[string]ComponentHealth) HealthSummary {
...
func (hc *SystemHealthChecker) calculateOverallStatus(summary HealthSummary, components map[string]ComponentHealth) ComponentHealthStatus {
...
func (hc *SystemHealthChecker) GetHealth(ctx context.Context) SystemHealth {
	return hc.CheckHealth(ctx)
}

// GetComponentHealth returns health of a specific component
func (hc *SystemHealthChecker) GetComponentHealth(name string) (ComponentHealth, bool) {
	hc.resultsMu.RLock()
...
func (hc *SystemHealthChecker) Start(ctx context.Context) {
...
func (hc *SystemHealthChecker) checkRuntime(ctx context.Context) ComponentHealth {
...
func (hc *SystemHealthChecker) checkGoroutines(ctx context.Context) ComponentHealth {
...
func (hc *SystemHealthChecker) AddDatabaseCheck(name string, db *sql.DB) {
	hc.AddCheck(HealthCheck{
		Name:     name,
...
func (hc *SystemHealthChecker) checkDatabase(ctx context.Context, db *sql.DB, name string) ComponentHealth {
	start := time.Now()
...
func (hc *SystemHealthChecker) AddRedisCheck(name string, client *redis.Client) {
	hc.AddCheck(HealthCheck{
		Name:     name,
...
func (hc *SystemHealthChecker) checkRedis(ctx context.Context, client *redis.Client, name string) ComponentHealth {
	start := time.Now()
...
func (hc *SystemHealthChecker) AddHTTPCheck(name, url string, expectedStatus int) {
	hc.AddCheck(HealthCheck{
		Name:     name,
...
func (hc *SystemHealthChecker) checkHTTP(ctx context.Context, url string, expectedStatus int, name string) ComponentHealth {
	start := time.Now()
...
func (hc *SystemHealthChecker) HTTPHandler() http.HandlerFunc {
...
func (hc *SystemHealthChecker) ReadinessHandler() http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		ctx := r.Context()
		health := hc.GetHealth(ctx)
