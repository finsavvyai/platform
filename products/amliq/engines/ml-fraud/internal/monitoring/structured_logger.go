...
// StructuredLoggingConfig holds logging configuration for structured logger
...
// StructuredLogLevel represents logging levels for structured logger
type StructuredLogLevel int

const (
	StructuredTraceLevel StructuredLogLevel = iota
	StructuredDebugLevel
	StructuredInfoLevel
	StructuredWarnLevel
	StructuredErrorLevel
	StructuredFatalLevel
	StructuredPanicLevel
)

func (l StructuredLogLevel) String() string {
	switch l {
	case StructuredTraceLevel:
		return "TRACE"
	case StructuredDebugLevel:
		return "DEBUG"
	case StructuredInfoLevel:
		return "INFO"
	case StructuredWarnLevel:
		return "WARN"
	case StructuredErrorLevel:
		return "ERROR"
	case StructuredFatalLevel:
		return "FATAL"
	case StructuredPanicLevel:
		return "PANIC"
	default:
		return "UNKNOWN"
	}
...
// StructuredLoggingContext provides context for logging
type StructuredLoggingContext struct {
	CorrelationID string
	RequestID     string
	UserID        string
	SessionID     string
...
func NewStructuredLogger(config StructuredLoggingConfig) *StructuredLogger {
...
func (sl *StructuredLogger) WithContext(ctx context.Context) *StructuredLogger {
	sl.mu.RLock()
	fields := make(map[string]interface{}, len(sl.fields)+5)
	...
	if reqID := ctx.Value("request_id"); reqID != nil {
		fields["request_id"] = reqID.(string)
	}
	if userID := ctx.Value("user_id"); userID != nil {
		fields["user_id"] = userID.(string)
	}
	if sessionID := ctx.Value("session_id"); sessionID != nil {
		fields["session_id"] = sessionID.(string)
	}
...
func (sl *StructuredLogger) log(level StructuredLogLevel, message string, args ...interface{}) {
	...
	if level == StructuredFatalLevel {
		os.Exit(1)
	} else if level == StructuredPanicLevel {
		panic(message)
	}
}
...
func (sl *StructuredLogger) formatText(entry StructuredLogEntry) string {
...
// ParseLogLevel parses a string into StructuredLogLevel
func ParseLogLevel(level string) StructuredLogLevel {
	switch strings.ToUpper(level) {
	case "TRACE":
		return StructuredTraceLevel
	case "DEBUG":
		return StructuredDebugLevel
	case "INFO", "INFORMATION":
		return StructuredInfoLevel
	case "WARN", "WARNING":
		return StructuredWarnLevel
	case "ERROR":
		return StructuredErrorLevel
	case "FATAL":
		return StructuredFatalLevel
	case "PANIC":
		return StructuredPanicLevel
	default:
		return StructuredInfoLevel
	}
}
