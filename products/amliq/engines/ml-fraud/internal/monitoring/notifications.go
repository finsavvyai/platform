...
// ChannelSender interface for sending notifications to channels
type ChannelSender interface {
	Send(ctx context.Context, message *NotificationMessage, config ChannelConfig) error
}
...
func (nm *NotificationManager) SendAlert(ctx context.Context, alert *ActiveAlert, config AlertNotificationConfig) error {
	message := nm.buildAlertMessage(alert, "fire")
	...
	sender, exists := nm.senders[config.Type]
	if !exists {
		return fmt.Errorf("unknown channel type: %s", config.Type)
	}

	return sender.Send(ctx, message, channelConfig)
}

// SendResolved sends a resolved notification
func (nm *NotificationManager) SendResolved(ctx context.Context, alert *ActiveAlert, config AlertNotificationConfig) error {
	message := nm.buildAlertMessage(alert, "resolved")
...
// buildAlertMessage builds a notification message from an alert
func (nm *NotificationManager) buildAlertMessage(alert *ActiveAlert, action string) *NotificationMessage {
