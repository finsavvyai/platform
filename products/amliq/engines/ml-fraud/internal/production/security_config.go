...
// SubnetConfig is defined in infrastructure_config.go
...
// VPCEndpoint is defined in infrastructure_config.go
...
type ServiceMeshAccessPolicy struct {
	...
	Selector  string                  `json:"selector"`
	Action    string                  `json:"action"`
	Rules     []ServiceMeshAccessRule `json:"rules"`
	When      []Condition             `json:"when"`
}

// ServiceMeshAccessRule holds access rule
type ServiceMeshAccessRule struct {
	From []Peer      `json:"from"`
	To   []Peer      `json:"to"`
	When []Condition `json:"when"`
}
...
func (psm *ProductionSecurityManager) GetComplianceReport(ctx context.Context, standardID string) (*ComplianceReport, error) {
	...
	for _, result := range psm.complianceChecker.results {
		...
		for _, _ = range standard.Requirements {
