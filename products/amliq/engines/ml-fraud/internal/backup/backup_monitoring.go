	...
	for key, value := range collectorMetrics {
		if strings.HasPrefix(key, "replication_") {
			...
			if len(parts) >= 3 {
				...
				// Ensure the map entry exists before modifying
				...
				rm := metrics.ReplicationStatus[region]

				switch metric {
				case "lag_seconds":
					rm.Lag = time.Duration(value) * time.Second
				case "success_rate":
					rm.SuccessRate = value
				case "errors":
					rm.Errors = int(value)
				case "status":
					if value == 1 {
				...
				// Assign the updated struct back to the map
				metrics.ReplicationStatus[region] = rm
			}
		}
	}
}
...
func (bmm *BackupMonitoringManager) processRestoreTestMetrics(metrics *BackupMetrics, collectorMetrics map[string]float64) {
	// Ensure the default entry exists before modifying
	if _, exists := metrics.RestoreTestResults["default"]; !exists {
		metrics.RestoreTestResults["default"] = RestoreTestMetrics{}
	}
	// Create a mutable copy to update
	rtm := metrics.RestoreTestResults["default"]
	...
	for key, value := range collectorMetrics {
		if strings.HasPrefix(key, "restore_test_") {
			...
			case "last_status":
				if value == 1 {
					rtm.LastTestStatus = "success"
				} else {
					rtm.LastTestStatus = "failed"
				}
			case "success_rate":
				rtm.SuccessRate = value
			case "average_duration_seconds":
				rtm.AverageDuration = time.Duration(value) * time.Second
			case "failure_count":
				rtm.FailureCount = int(value)
			}
		}
	}
	// Assign the updated struct back to the map
	metrics.RestoreTestResults["default"] = rtm
}
...
func (bmm *BackupMonitoringManager) processSystemHealthMetrics(metrics *BackupMetrics, collectorMetrics map[string]float64) {
	...
	for key, value := range collectorMetrics {
		if strings.HasPrefix(key, "component_") {
			...
			if value >= 80 { // Assuming score is out of 100
