...
func NewClient(config *Config) (*Client, error) {
	...
	clientOptions := influxdb2.DefaultOptions().
		SetHTTPRequestTimeout(uint(config.Timeout.Seconds())).
		...
		SetPrecision(time.Duration(time.Nanosecond)) // config.Precision is string, need conversion if SetPrecision expects duration or string. Usually SetPrecision takes duration or string.
	...
	_, err := client.OrganizationsAPI().FindOrganizationByName(ctx, config.Org)
	if err != nil {
