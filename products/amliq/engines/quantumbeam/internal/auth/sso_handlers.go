...
func (h *SSOHandlers) exchangeCodeForToken(c *gin.Context, provider, code string) (string, error) {
	...
	// 1. Get OAuth2 config for the provider
	// 2. Exchange code for token using oauth2.Config.Exchange()
	// 3. Extract ID token from the token response

	// For now, return the code as-is (assuming it's already an ID token)
	// This would need proper implementation based on the OIDC provider
	return code, nil
}
