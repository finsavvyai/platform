...
type AlertMetrics struct {
	AlertsTotal       prometheus.Counter   `json:"alerts_total"`
	AlertsBySeverity   *prometheus.CounterVec `json:"alerts_by_severity"`
	AlertsByStatus    *prometheus.CounterVec `json:"alerts_by_status"`
	IncidentsTotal    prometheus.Counter   `json:"incidents_total"`
	IncidentsOpen     prometheus.Gauge     `json:"incidents_open"`
...
func (am *AlertManager) ProcessAlert(alert AnomalyAlert) error {
	...
	// Update metrics
	am.metrics.AlertsTotal.WithLabelValues(
		prometheus.Labels{
...
func (am *AlertManager) findMatchingRules(alert AnomalyAlert) []*AlertRule {
...
func (am *AlertManager) alertMatchesRule(alert AnomalyAlert, rule *AlertRule) bool {
	// Check severity
...
func (am *AlertManager) createOrUpdateIncident(alert AnomalyAlert, rules []*AlertRule) (*Incident, error) {
	am.incidentStore.mu.Lock()
	...
	if incident == nil {
		incident = &Incident{
			...
			Severity:    determineIncidentSeverity(alert, rules),
			Status:      "new",
			State:       "open",
	...
	incident.Alerts = append(incident.Alerts, alert)
		...
		newSeverity := determineIncidentSeverity(alert, rules)
	...
	// Update metrics
	am.metrics.IncidentsTotal.WithLabelValues(
		prometheus.Labels{
			"severity": incident.Severity,
		},
	).Inc()

	am.metrics.IncidentsOpen.WithLabelValues(
		prometheus.Labels{
			"severity": incident.Severity,
		},
...
func (am *AlertManager) sendIncidentNotifications(incident *Incident) error {
	...
	if am.config.Notification.Email.Enabled {
		if err := am.sendEmailNotification(incident); err != nil {
			amIncidentLogger.Printf("Failed to send email notification: %v", err)
		}
	}
	...
	for _, webhook := range am.config.Notification.Webhook {
...
func (am *AlertManager) incidentManagementLoop() {
	ticker := time.NewTicker(am.config.IncidentConfig.AckTimeout)
	defer ticker.Stop()
...
func (am *AlertManager) checkIncidentAging() {
	am.incidentStore.mu.RLock()
	defer am.incidentStore.RUnlock()

	for _, incident := range am.incidentIncidents {
...
func determineIncidentSeverity(alert AnomalyAlert, rules []*AlertRule) string {
	// Start with alert severity
	severity := alert.Severity

	// Check if any rule specifies higher severity
	for _, rule := range rules {
		if rule.Severity != "" && shouldUpgradeSeverity(severity, rule.Severity) {
...
func (am *AlertManager) ResolveIncident(id string, user string, resolution string) error {
	...
	incident, exists := am.incident.incidents[id]
	if !exists {
		return fmt.Errorf("incident not found: %s", id)
	}
