...
// HealthChecker holds health checker
...
func (psm *ProductionServiceManager) setupHealthChecks(service ProductionService) error {
	...
	if service.HealthCheck.Path != "" {
		healthChecker.Checks = append(healthChecker.Checks, HealthCheck{
			Name:    "http",
			Type:    "http",
...
func (psm *ProductionServiceManager) performHealthCheck(ctx context.Context, serviceName string, check HealthCheck) HealthCheckResult {
	startTime := time.Now()
