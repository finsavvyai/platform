...
func (pmm *ProductionMonitoringManager) CreateDashboard(dashboard Dashboard) error {
	pmm.dashboards[dashboard.Name] = dashboard
	pmm.logger.Printf("Created dashboard: %s", dashboard.Name)
	return nil
}

// CreateAlert creates a new alert
func (pmm *ProductionMonitoringManager) CreateAlert(alert Alert) error {
	pmm.alerts[alert.Name] = alert
	pmm.logger.Printf("Created alert: %s", alert.Name)
	return nil
}

// CreateHealthChecker creates a new health checker
func (pmm *ProductionHealthChecker) CreateHealthChecker(checker HealthChecker) error {
	pmm.healthCheckers[checker.GetName()] = checker
	pmm.logger.Printf("Created health checker: %s", checker.GetName())
	return nil
}
