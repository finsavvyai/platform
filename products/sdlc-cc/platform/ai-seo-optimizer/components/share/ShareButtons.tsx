...
const ShareButtons = ({ message, url, score }: ShareButtonsProps) => {
  ...
  const shareToLinkedIn = () => {
    const text = encodeURIComponent(fullMessage);
    window.open(`https://www.linkedin.com/sharing/share-offsite/?url=${encodeURIComponent(url)}&summary=${text}`, '_blank', 'noopener');
  };

  const shareNative = async () => {
    if (navigator.share) {
      await navigator.share({ title: `My AI Visibility Score: ${score}/100`, text: fullMessage, url });
    } else {
      handleCopy();
    }
  };
    ...
    <div className="space-y-3">
      <div className="grid grid-cols-2 gap-3">
        <motion.button
          ...
          whileHover={{ y: -1 }}
          whileTap={{ scale: 0.97 }}
          className="flex items-center justify-center gap-2 px-4 py-3 rounded-xl bg-slate-900 text-white text-sm font-medium hover:bg-slate-800 transition-colors"
        >
          <XIcon />
          Post on X
        </motion.button>
