...
if command -v dig >/dev/null 2>&1; then
    if dig +short "${DOMAIN_NAME:-sdlc.cc}" >/dev/null 2>&1; then
        print_success "Domain ${DOMAIN_NAME:-sdlc.cc} resolves"
    else
        print_warning "Domain ${DOMAIN_NAME:-sdlc.cc} does not resolve (DNS not configured yet)"
    fi
else
