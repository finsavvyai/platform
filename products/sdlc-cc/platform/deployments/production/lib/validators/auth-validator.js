...
class AuthValidator {
  ...
  async checkCloudflareAuth() {
    ...
    try {
      ...
      if (error.message.includes('not authenticated') || 
        ...
        errors.push('Not authenticated with Cloudflare. Run "wrangler auth login" to authenticate.');
      } else {
  ...
  async validateApiToken() {
    ...
    try {
      ...
      if (!apiToken) {
        // API token is optional if using wrangler auth login
        warnings.push('CLOUDFLARE_API_TOKEN not set (using wrangler auth login authentication)');
        return { valid: true, errors, warnings };
      }
