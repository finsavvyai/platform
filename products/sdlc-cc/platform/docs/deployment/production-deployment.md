...
export const productionConfig = {
  ...
  security: {
    ...
    cors: {
      allowedOrigins: ['https://app.sdlc.cc', 'https://admin.sdlc.cc'],
...
BLUE_ENV="blue"
GREEN_ENV="green"
CURRENT_ENV=$(curl -s https://api.sdlc.cc/health | jq -r '.environment')
...
for i in {1..10}; do
  if curl -f https://$TARGET_ENV.api.sdlc.cc/health > /dev/null 2>&1; then
...
export async function performHealthChecks() {
  const checks = [
    {
      name: 'API Gateway',
      url: 'https://api.sdlc.cc/health',
      expectedStatus: 200
    },
    {
      name: 'Authentication',
      url: 'https://api.sdlc.cc/auth/health',
      expectedStatus: 200
    },
    {
      name: 'Document Service',
      url: 'https://api.sdlc.cc/documents/health',
      expectedStatus: 200
    },
    {
      name: 'RAG Service',
      url: 'https://api.sdlc.cc/rag/health',
      expectedStatus: 200
    },
    {
...
export async function validatePerformance() {
  ...
  for (const endpoint of endpoints) {
    ...
    for (let i = 0; i < 100; i++) {
      ...
      await fetch(`https://api.sdlc.cc${endpoint.path}`, {
...
For additional support, contact the DevOps team at devops@sdlc.cc or check the troubleshooting documentation.