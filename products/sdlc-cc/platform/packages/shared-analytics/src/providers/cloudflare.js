...
export class CloudflareProvider {
    ...
    async initialize(config) {
        try {
            ...
            this.kvNamespace = globalThis.ANALYTICS_KV;
            this.d1Database = globalThis.ANALYTICS_DB;
            this.analyticsEngine = globalThis.AnalyticsEngine;
    ...
    async storeEventInD1(event) {
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?)
    `);
        await stmt.bind(event.id, event.type, event.userId || null, event.sessionId, event.timestamp, JSON.stringify(event.data), JSON.stringify(event.metadata)).run();
    }
    /**
    ...
    buildSQLFilters(filters) {
        ...
        if (filters.products && filters.products.length > 0) {
            const products = filters.products.map((p) => `'${p}'`).join(', ');
     ...
     */
    async createDailyAggregates(date) {
        if (!this.d1Database)
            return;
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).bind(date, 'sdlc-platform', // TODO: Get from config
        metrics.totalEvents, metrics.uniqueUsers, metrics.totalSessions, metrics.bounceRate, metrics.avgSessionDuration, metrics.topPages.reduce((sum, page) => sum + page.views, 0), metrics.conversionRate, metrics.errorRate).run();
    }
}
