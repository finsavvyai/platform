...
interface ActivityEvent {
  id: string;
  type: 'deployment' | 'commit' | 'api_call' | 'user_action' | 'security' | 'billing' | 'system';
  title: string;
  description: string;
  user: string;
  timestamp: string;
  product: 'sdlc' | 'rag' | 'gateway' | 'billing' | 'system';
...
export const ActivityFeedWidget: React.FC<ActivityFeedWidgetProps> = ({
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      const eventTypes: ActivityEvent['type'][] = ['deployment', 'api_call', 'commit', 'user_action'];
      const products: ActivityEvent['product'][] = ['sdlc', 'rag', 'gateway', 'billing'];
  ...
  const getProductBadge = (product: ActivityEvent['product']) => {
    const badges: Record<ActivityEvent['product'], { label: string; color: string }> = {
      sdlc: { label: 'SDLC', color: 'bg-blue-100 text-blue-700 dark:bg-blue-900 dark:text-blue-300' },
      rag: { label: 'RAG', color: 'bg-purple-100 text-purple-700 dark:bg-purple-900 dark:text-purple-300' },
      gateway: { label: 'Gateway', color: 'bg-green-100 text-green-700 dark:bg-green-900 dark:text-green-300' },
  ...
  const getSeverityColor = (severity?: ActivityEvent['severity']) => {
    if (!severity) return 'border-gray-200 dark:border-gray-700';
    const colors: Record<NonNullable<ActivityEvent['severity']>, string> = {
                    ...
                    <div className="flex items-center space-x-2 text-xs text-gray-500 dark:text-gray-500">
                      ...
                      {activity.metadata && Object.keys(activity.metadata).length > 0 && (
                        <>
                          <span>•</span>
                          <button
                            onClick={() => console.log('Show details:', activity.metadata)}
