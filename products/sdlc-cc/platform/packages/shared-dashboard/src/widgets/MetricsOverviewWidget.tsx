...
export const MetricsOverviewWidget: React.FC<MetricsOverviewWidgetProps> = ({
    ...
    <div className={`card ${className}`}>
      ...
      <div className="card-body">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mb-6">
          {metrics.map((metric) => (
            <div key={metric.id} className="p-4 bg-gray-50 dark:bg-gray-900 rounded-lg">
              ...
              <div className="flex items-center space-x-2 mb-2">
                ...
                {metric.change && (
                  <div className={`flex items-center space-x-1 text-xs ${
                    metric.change.type === 'increase' ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'
                  }`}>
              ...
              {metric.target && (
                <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2">
                  <div
                    className={`h-2 rounded-full transition-all duration-300 ${getProgressColor(getProgressPercentage(Number(metric.value), metric.target))}`}
                    style={{ width: `${getProgressPercentage(Number(metric.value), metric.target)}%` }}
                  ></div>
                </div>
              )}
              {metric.change && (
