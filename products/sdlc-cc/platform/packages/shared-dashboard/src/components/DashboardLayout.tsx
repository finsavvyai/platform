...
export const DashboardLayout: React.FC<DashboardLayoutProps> = ({
  ...
  const [activeTheme, setActiveTheme] = useState<'light' | 'dark' | 'auto'>(
    typeof layout?.theme === 'string' ? layout.theme : layout?.theme?.mode ?? 'auto'
  );
  ...
  const handleSidebarToggle = () => {
    ...
    onLayoutChange?.({
      sidebar: {
        pinnedItems: layout?.sidebar?.pinnedItems ?? [],
  ...
  return (
    <div className={`dashboard-layout min-h-screen flex flex-col ${className}`}>
      {/* Status Bar */}
      <StatusBar
        user={user}
        theme={activeTheme}
      ...
      {/* Global Styles */}
      {/* @ts-expect-error - styled-jsx global styles */}
      <style jsx global>{`
        .dashboard-layout {
          --sidebar-width: 280px;
