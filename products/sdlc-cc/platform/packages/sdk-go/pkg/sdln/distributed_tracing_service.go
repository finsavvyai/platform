...
// SamplingCounter tracks sampling counts
...
type TracePattern struct {
	ID          string                 `json:"id"`
	Name        string                 `json:"name"`
	Type        TracePatternType            `json:"type"`
...
// TracePatternType defines pattern types
type TracePatternType string

const (
	TracePatternTypeNormal     TracePatternType = "normal"
	TracePatternTypeSlow       TracePatternType = "slow"
	TracePatternTypeError      TracePatternType = "error"
	TracePatternTypeTimeout    TracePatternType = "timeout"
	TracePatternTypeBottleneck TracePatternType = "bottleneck"
	TracePatternTypeCascading  TracePatternType = "cascading"
	TracePatternTypeRepeated   TracePatternType = "repeated"
)
...
func (dts *DistributedTracingService) matchesQuery(trace *TraceSummary, query *TraceSearchQuery) bool {
	...
	// Check time range
	if query.StartTime != nil && time.Time(trace.StartTime).Before(time.Time(*query.StartTime)) {
		return false
	}
	if query.EndTime != nil && time.Time(trace.EndTime).After(time.Time(*query.EndTime)) {
		return false
	}
...
func (dts *DistributedTracingService) sortTraces(traces []*TraceSummary, sortBy string, desc bool) {
	sort.Slice(traces, func(i, j int) bool {
		...
		case "start_time":
			less = time.Time(traces[i].StartTime).Before(time.Time(traces[j].StartTime))
		default:
			less = time.Time(traces[i].StartTime).Before(time.Time(traces[j].StartTime))
		}
...
func (dts *DistributedTracingService) buildTraceSummary(traceID string) *TraceSummary {
	...
	for _, spanID := range spanIDs {
		...
		// Set start/end times
		if time.Time(summary.StartTime).IsZero() || time.Time(span.StartTime).Before(time.Time(summary.StartTime)) {
			summary.StartTime = span.StartTime
		}
		if time.Time(summary.EndTime).IsZero() || time.Time(span.EndTime).After(time.Time(summary.EndTime)) {
		...
		if comp, exists := components[span.Component]; exists {
			...
			if span.Status == "error" {
				components[span.Component].ErrorRate = 1
			}
		}
	...
	summary.Duration = time.Time(summary.EndTime).Sub(time.Time(summary.StartTime))
	...
	for _, comp := range components {
		comp.ErrorRate = comp.ErrorRate / float64(comp.SpanCount)
...
func (dts *DistributedTracingService) classifyTrace(trace *TraceSummary) TraceClassification {
	classification := TraceClassification{
		Pattern:     string(TracePatternTypeNormal),
		Confidence:  1.0,
		Anomalies:   []string{},
		Bottlenecks: []string{},
		Issues:      []string{},
	...
	if trace.ErrorCount > 0 {
		classification.Pattern = string(TracePatternTypeError)
	...
	if trace.Duration > time.Second*5 {
		if classification.Pattern == string(TracePatternTypeError) {
			classification.Pattern = "slow_error"
		} else {
			classification.Pattern = string(TracePatternTypeSlow)
		}
	...
	if trace.ErrorCount > 3 && trace.ServiceCount > 3 {
		classification.Pattern = string(TracePatternTypeCascading)
	...
	if trace.Duration > time.Second*5 {
		...
		if pattern, exists := dts.analytics.patterns[patternID]; exists {
			...
			dts.analytics.patterns[patternID] = &TracePattern{
				ID:          patternID,
				Name:        "Slow Trace",
				Type:        TracePatternTypeSlow,
				Description: "Trace duration exceeds 5 seconds",
				Frequency:   1,
				Impact:      0.7,
				Created:     NewTimestampNow(),
			}
		}
	}
...
func (dts *DistributedTracingService) GetServiceMap(ctx context.Context, tenantID string, timeRange *TimeRange) (*ServiceMap, error) {
	...
	for _, trace := range dts.traces.traces {
		// Check time range
		if timeRange != nil {
			if time.Time(trace.StartTime).Before(time.Time(timeRange.From)) || time.Time(trace.EndTime).After(time.Time(timeRange.To)) {
				continue
			}
		}
