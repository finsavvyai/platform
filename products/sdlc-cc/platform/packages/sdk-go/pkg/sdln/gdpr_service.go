...
func (gm *GDPRManager) validateRequest(req *DataSubjectRequest) error {
	...
	if time.Until(time.Time(req.DueDate)) > 30*24*time.Hour {
...
func (gm *GDPRManager) verifyIdentity(proof *IdentityProof) error {
	...
	if time.Until(time.Time(proof.ExpiryDate)) < 0 {
...
func (gm *GDPRManager) processAccessRequest(ctx context.Context, req *DataSubjectRequest) (*DataSubjectRequest, error) {
	...
	query := &AuditQuery{
		UserIDs:   []string{req.DataSubjectID},
		StartTime: func() *Timestamp { t := NewTimestamp(time.Time{}); return &t }(),
		EndTime:   func() *Timestamp { t := NewTimestamp(time.Now().UTC()); return &t }(),
		Limit:     10000,
	}
...
func (gm *GDPRManager) convertToCSV(logs []*AuditLog) string {
	...
	for _, log := range logs {
		csv += fmt.Sprintf("%s,%s,%s,%s,\"%s\"\n",
			log.ID,
			time.Time(log.Timestamp).Format(time.RFC3339),
			log.EventType,
			log.Action,
