...
func (s *WebSocketService) Connect(ctx context.Context) (*WSConnection, error) {
	...
	req.Header.Set("WSConnection", "keep-alive")
...
func (s *WebSocketService) ConnectWithAuth(ctx context.Context, token string) (*WSConnection, error) {
	// Build WebSocket URL
	wsURL, err := url.Parse(s.client.config.BaseURL)
	if err != nil {
		return nil, fmt.Errorf("failed to parse base URL: %w", err)
...
func (c *WSConnection) Subscribe(req *SubscribeRequest) error {
	message := Message{
		Type: "subscribe",
...
func (c *WSConnection) Unsubscribe(req *UnsubscribeRequest) error {
	message := Message{
		Type: "unsubscribe",
		Data: map[string]interface{}{
			"events":    req.Events,
...
func (c *WSConnection) Send(messageType string, data map[string]interface{}) error {
	message := Message{
		Type:      messageType,
		Data:      data,
		Timestamp: Timestamp(time.Now()),
...
func (c *WSConnection) Events() <-chan []byte {
	return c.receive
}

// Errors returns a channel for receiving errors
func (c *WSConnection) Errors() <-chan error {
	return c.errorChan
}
...
func (c *WSConnection) SetMessageHandler(handler func(message []byte)) {
	c.mutex.Lock()
...
func (c *WSConnection) SetErrorHandler(handler func(err error)) {
	c.mutex.Lock()
	defer c.mutex.Unlock()
	c.onError = handler
}
...
func (c *WSConnection) SetCloseHandler(handler func()) {
	c.mutex.Lock()
	defer c.mutex.Unlock()
	c.onClose = handler
}
...
func (c *WSConnection) sendMessage(message Message) error {
