...
func (ve *ValidationErrors) Addf(field, format string, args ...interface{}) {
	ve.Errors = append(ve.Errors, ValidationError{
		Field:   field,
...
// AddWithValue adds a validation error with a value
func (ve *ValidationErrors) AddWithValue(field, message string) {
	ve.Errors = append(ve.Errors, ValidationError{
		Field:   field,
		Message: message,
	})
...
func (te *TimeoutError) IsTimeout() bool {
	return true
}
...
func ErrTimeout(operation string, timeout time.Duration) *TimeoutError {
	return &TimeoutError{
		Operation: operation,
		Timeout:   timeout,
	}
