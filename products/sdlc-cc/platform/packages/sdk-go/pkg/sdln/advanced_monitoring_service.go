...
// SecurityMonitoringService provides security monitoring
type SecurityMonitoringService struct{ *BaseService }

// BusinessIntelligenceService provides BI capabilities
type BusinessIntelligenceService struct{ *BaseService }

// SLAMonitoringService provides SLA monitoring
type SLAMonitoringService struct{ *BaseService }

// IncidentResponseService manages incident response
type IncidentResponseService struct{ *BaseService }

// CapacityPlanningService manages capacity planning
type CapacityPlanningService struct{ *BaseService }
...
func NewAdvancedMonitoringService(client *Client) *AdvancedMonitoringService {
	...
	svc.perfMonitor = NewPerformanceMonitoringService(svc.client)
...
func (ma *MetricsAggregator) AggregateMetrics(ctx context.Context, tenantID string, metrics []Metric) (*AggregationResult, error) {
	...
	result.Aggregations["aggregation_time"] = time.Since(time.Time(result.Timestamp)).Seconds()
...
func (ad *AnomalyDetector) seasonalDetection(series MetricSeries, threshold AnomalyThreshold) []Anomaly {
	...
	for _, point := range series.Points {
		timeKey := time.Time(point.Timestamp).Format("15:04") // Hour:Minute
	...
	for _, point := range series.Points {
		timeKey := time.Time(point.Timestamp).Format("15:04")
...
func (ad *AnomalyDetector) aggregateAnomalies(anomalies []Anomaly) []Anomaly {
	...
	for _, anomaly := range anomalies {
		key := time.Time(anomaly.Timestamp).Format(time.RFC3339)
...
func (iam *IntelligentAlertManager) groupAlert(alert *Alert, rule CorrelationRule) {
	...
	for id, activeAlert := range iam.activeAlerts {
		if time.Time(activeAlert.Alert.StartsAt).After(windowStart) {
...
func (iam *IntelligentAlertManager) estimateLoss(alert *Alert) float64 {
	...
	duration := time.Since(time.Time(alert.StartsAt)).Hours()
...
func (iam *IntelligentAlertManager) shouldEscalate(activeAlert *ActiveAlert, condition EscalationCondition) bool {
	...
	case "duration":
		duration := time.Since(time.Time(activeAlert.Alert.StartsAt))
