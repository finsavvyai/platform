...
func TestDefaultConfig(t *testing.T) {
	...
	assert.Equal(t, "https://api.sdlc.cc", config.BaseURL)
...
func TestURLBuilder_Build(t *testing.T) {
	// Basic URL building
	url := NewURLBuilder("https://api.sdlc.cc")
	assert.Equal(t, "https://api.sdlc.cc", url.MustBuild())

	// Add path segments
	url.Path("v1", "users").Path("123")
	assert.Equal(t, "https://api.sdlc.cc/v1/users/123", url.MustBuild())

	// Add query parameters
	url.Query("page", "1").Query("limit", "10")
	assert.Equal(t, "https://api.sdlc.cc/v1/users?page=1&limit=10", url.MustBuild())

	// Add fragment
	url.Fragment("section1").Fragment("subsection1")
	assert.Equal(t, "https://api.sdlc.cc/v1/users?page=1&limit=10#section1#subsection1", url.MustBuild())
}
...
func TestClient_RealAPIOperations(t *testing.T) {
	...
	config := &Config{
		BaseURL:      GetEnv("API_URL", "https://api.sdlc.cc"),
		Timeout:      30 * time.Second,
		Debug:        false,
	}
...
func createTestClient() *Client {
	config := &Config{
		BaseURL: "https://test-api.sdlc.cc",
		Timeout: 30 * time.Second,
		Debug:   false,
	}
...
func createTestTenant(t *testing.T) (*Tenant, error) {
	...
	tenant, err := client.Tenants.Create(context.Background(), &CreateTenantRequest{
		...
		Domain:  fmt.Sprintf("test-%d.sdlc.cc", time.Now().Unix()),
...
func TestRequestBuilder_Build(t *testing.T) {
	...
	// Test URL
	req, err := builder.URL("https://api.sdlc.cc/v1/users").Build()
	assert.NoError(t, err)
	assert.Equal(t, "https://api.sdlc.cc/v1/users", req.URL())
	...
	assert.Equal(t, "https://api.sdlc.cc/v1/users", httpReq.URL())
...
func TestURLBuilder_BuildWithQuery(t *testing.T) {
	builder := NewURLBuilder("https://api.sdlc.cc")

	// Add path
	builder.Path("v1", "users").Path("123").Path("active"))
	assert.Equal(t, "https://api.sdlc.cc/v1/users/123/active", builder.MustBuild())
	...
	builder.Query("page", "1").
		...
		Query("order", "desc").
		Build()
	assert.Equal(t, "https://api.sdlc.cc/v1/users/123/active?page=1&limit=10&sort_by=created_at&order=desc", builder.MustBuild())
}
...
func TestConfig_CustomConfiguration(t *testing.T) {
	config := &Config{
		BaseURL:            "https://custom.api.sdlc.cc",
	Timeout:            60 * time.Second,
		MaxIdleConns:       50,
		MaxIdleConnsPerHost: 10,
		IdleConnTimeout:     120 * time.Second,
