...
func (b *TimeSeriesBuffer) GetPointsInRange(start, end Timestamp) []MetricPoint {
	...
	for _, point := range points {
		if !time.Time(point.Timestamp).Before(time.Time(start)) && !time.Time(point.Timestamp).After(time.Time(end)) {
...
func (c *PerformanceMetricsCollector) GetMetrics(name string, timeRange TimestampRange) []MetricPoint {
	...
	if buffer, ok := c.metrics[name]; ok {
		...
		return buffer.GetPointsInRange(from, to)
	}
	return nil
}
...
func (ac *AlertCorrelator) calculateSimilarity(a, b Alert) float64 {
	...
	timeDiff := time.Time(a.StartsAt).Sub(time.Time(b.StartsAt))
...
func (nr *NoiseReducer) ShouldSuppress(alert Alert) bool {
	...
	timeSinceLast := time.Time(alert.StartsAt).Sub(time.Time(lastAlert))
...
func (ta *TrendAnalyzer) AnalyzeTrend(points []MetricPoint) TrendResult {
	...
	sort.Slice(points, func(i, j int) bool {
		return time.Time(points[i].Timestamp).Before(time.Time(points[j].Timestamp))
	})
	...
	for _, point := range points {
		x := float64(time.Time(point.Timestamp).Sub(startTime))
		y := point.Value
		sumX += x
		sumY += y
		sumXY += x * y
...
func (aa *AlertAggregator) AggregateAlerts(alerts []Alert) AlertAggregation {
	...
	for _, alert := range alerts {
		...
		// Count by hour
		hour := time.Time(alert.StartsAt).Format("2006-01-02 15:00")
		...
		if alert.Status == "resolved" && alert.EndsAt != nil {
			resolutionTime := time.Time(*alert.EndsAt).Sub(time.Time(alert.StartsAt))
	...
	if len(alerts) > 0 {
		...
		for _, alert := range alerts {
			if time.Time(alert.StartsAt).Before(time.Time(oldestAlert.StartsAt)) {
				oldestAlert = alert
			}
		}

		duration := now.Sub(time.Time(oldestAlert.StartsAt))
		if duration > 0 {
