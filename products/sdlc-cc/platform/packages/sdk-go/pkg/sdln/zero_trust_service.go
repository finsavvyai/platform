...
func (s *ZeroTrustService) detectAnomalies(ctx context.Context, request *TrustRequest) *ZTAnomalyDetectionResult {
	result := &ZTAnomalyDetectionResult{
		Score: 0.0,
	}
...
// SignatureValidationResult captures the outcome of signature validation
...
func (rs *RequestSigner) ValidateSignature(request *TrustRequest) *SignatureValidationResult {
	if request == nil || len(request.Signature) == 0 {
		return &SignatureValidationResult{Valid: false, Reason: "missing signature"}
	}
...
type ZTAnomalyDetectionResult struct {
