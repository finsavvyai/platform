...
import type { ServiceHealthStatus, WebhookPayload, UnanalyzedFeedbackRow } from './types';
...
app.use('*', logger());
app.use('*', cors({
  origin: ['https://sdlc.cc', 'https://app.sdlc.cc', 'https://beta.sdlc.cc'],
...
app.get('/api/beta/health', async (c) => {
  ...
  const isHealthy = Object.values(health.services).every((s: ServiceHealthStatus) => s.status === 'healthy');
...
app.scheduled('/api/beta/jobs/feedback-analysis', async (c) => {
  ...
  const unanalyzed = await mcp.db
    ...
    .all<UnanalyzedFeedbackRow>();

  if (unanalyzed.results?.length > 0) {
    await feedbackAnalysis.processFeedbackBatch(
      unanalyzed.results.map((f: UnanalyzedFeedbackRow) => f.id)
    );
  }
...
async function checkKVHealth(): Promise<ServiceHealthStatus> {
  try {
...
async function checkLLMHealth(): Promise<ServiceHealthStatus> {
  try {
