...
describe('Utils', () => {
  ...
  describe('formatDateTime', () => {
    it('formats date and time correctly', () => {
      ...
      // en-US locale uses 12-hour clock by default
      expect(result).toMatch(/January 15, 2024.*02:30\s*PM/)
    })
  })
  ...
  describe('formatCurrency', () => {
    ...
    it('handles different currencies', () => {
      // Intl may emit NBSP between symbol and digits; normalize before compare.
      const result = formatCurrency(1234.56, 'EUR').replace(/ /g, ' ')
      expect(result).toBe('€1,234.56')
    })

    it('handles different locales', () => {
      // Intl uses NBSP ( ) as the separator in de-DE; normalize.
      const result = formatCurrency(1234.56, 'USD', 'de-DE').replace(/ /g, ' ')
      expect(result).toBe('1.234,56 $')
    })
  })
