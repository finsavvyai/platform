...
function SidebarSection({ section, isCollapsed, permissions }: SidebarSectionProps) {
  ...
  const [expanded, setExpanded] = React.useState(() => {
    ...
    return section.items.some((item: NavItem) => item.href === pathname)
  })

  const hasVisibleItems = section.items.some((item: NavItem) =>
      ...
      >
        ...
        <div className="border-t p-2">
          <Button
            ...
            onClick={() => useUIStore.getState().setSidebarCollapsed(!sidebar.isCollapsed)}
          >
            {sidebar.isCollapsed ? (
