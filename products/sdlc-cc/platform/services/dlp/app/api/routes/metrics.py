...
async def component_metrics(
    ...
    try:
        ...
        # Agent Booster metrics
        try:
            booster = get_fast_pii_detector()
            booster_snap = booster.metrics.snapshot()
            component_data["components"]["agent_booster"] = {
                "status": "healthy" if booster.enabled else "disabled",
                "enabled": booster.enabled,
                "patterns_loaded": len(booster._patterns),
                "fast_path_count": booster_snap.fast_path_count,
                "llm_fallback_count": booster_snap.llm_fallback_count,
                "fast_path_ratio": round(booster_snap.fast_path_ratio, 4),
                "avg_fast_path_latency_us": round(
                    booster_snap.avg_fast_path_latency_us, 2
                ),
            }
        except Exception as e:
            component_data["components"]["agent_booster"] = {
...
@router.get("/agent-booster")
async def agent_booster_metrics(
    current_user: dict = Depends(get_current_user),
):
    """
    Get Agent Booster fast-path detection metrics.

    Returns fast-path vs LLM-path ratio, latency stats,
    and per-pattern hit counts.
    """
    try:
        detector = get_fast_pii_detector()
        return {
        ...
        logger.error(f"Failed to get Agent Booster metrics: {e}")
        return {"error": str(e), "timestamp": datetime.utcnow().isoformat()}
