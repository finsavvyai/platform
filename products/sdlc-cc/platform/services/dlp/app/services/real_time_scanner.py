...
from app.services.fast_pii_detector import get_fast_pii_detector
from app.services.presidio_detector import PIIExtractionResult, get_presidio_detector
from app.services.regex_engine import PatternMatchResult, get_regex_engine
...
class ScanCache:
    ...
    async def get(self, content_hash: str) -> ScanResult | None:
...
class ScanQueue:
    ...
    async def get(self) -> ScanTask | None:
...
class RealTimeScanner:
    ...
    def __init__(self):
        ...
        self.fast_pii_detector = get_fast_pii_detector()
    ...
    async def _scan_synchronous(
        self, task: ScanTask, content_hash: str | None
    ) -> ScanResult:
    ...
    async def _execute_scan(self, task: ScanTask) -> ScanResult:
        ...
        # Agent Booster: fast-path for deterministic PII detection
        # If all PII can be handled by regex, skip Presidio/ML entirely
        if self.fast_pii_detector.can_handle(content):
            booster_result = await self._run_fast_pii_scan(
                task, content, start_time
            )
            if booster_result is not None:
                return booster_result
            # Fast path returned None means it found no PII but we
            # still want the full pipeline for classification/rules

        # Record LLM fallback in metrics
        self.fast_pii_detector.metrics.record_llm_fallback(
        ...
        metadata = ScanMetadata(
            ...
            additional_metadata={
                ...
                "agent_booster": self.fast_pii_detector.get_metrics(),
            },
        )
    ...
    async def _run_fast_pii_scan(
        self, task: ScanTask, content: str, start_time: float
    ) -> ScanResult | None:
        """
        Agent Booster fast-path: detect and mask PII using regex only.

        Returns a ScanResult if PII was found and handled, or None
        to fall through to the full pipeline.
        """
        try:
            fast_matches = self.fast_pii_detector.detect(content)
            if not fast_matches:
                return None  # No PII found, let full pipeline run

            # Convert fast matches to violations
            all_violations = []
            for match in fast_matches:
                violation = ViolationInfo(
                    id=f"{task.id}-booster-{match.start}",
                    violation_type=f"BOOSTER_{match.pii_type.value}",
                    severity=self._booster_severity(match.pii_type.value),
                    confidence=match.confidence,
                    content_type=task.content_type or "text/plain",
                    content_path=task.content_path,
                    detected_value=match.matched_text,
                    entity_type=match.pii_type.value,
                    pattern_name="agent_booster",
                    context=content[
                    ...
                    metadata={
                        "scan_id": task.id,
                        "tenant_id": task.tenant_id,
                        "detection_path": "agent_booster_fast",
                    },
                )
            ...
            return ScanResult(
                ...
                violations_by_severity=dict(violations_by_severity),
                violations_by_type=dict(violations_by_type),
                risk_score=max((v.confidence for v in all_violations), default=0),
                risk_level=RiskLevel.HIGH if all_violations else RiskLevel.LOW,
                violations=all_violations,
                metadata=ScanMetadata(
                    scan_id=task.id,
                    tenant_id=task.tenant_id,
                    ...
                    additional_metadata={
                        "detection_path": "agent_booster_fast",
                        "booster_matches": len(fast_matches),
                    },
                ),
            )
        except Exception as e:
            logger.warning(f"Agent Booster fast-path failed: {e}")
            self.fast_pii_detector.metrics.record_error()
            return None

    @staticmethod
    def _booster_severity(pii_type: str) -> ViolationSeverity:
        """Map PII type to violation severity."""
        critical = {"SSN", "CREDIT_CARD"}
        high = {"EMAIL", "PHONE", "DATE_OF_BIRTH"}
    ...
    def _run_content_classification(self, content: str):
        ...
        try:
            ...
            # Return mock result
            from app.models.schemas import ClassificationResult, ContentType
