...
func NewDocumentRepository(db *gorm.DB) DocumentRepository {
	base := NewBaseRepository[models.Document](db)
	return &DocumentRepositoryImpl{
		BaseRepositoryImpl: base.(*BaseRepositoryImpl[models.Document]),
	}
}
...
func (r *DocumentRepositoryImpl) GetDocumentStatistics(ctx context.Context, tenantID string) (*DocumentStats, error) {
	...
	var totalSize int64
	var avgSize *int64
	row := r.db.WithContext(ctx).Model(&models.Document{}).
		Where("tenant_id = ?", tenantUUID).
		Select("COALESCE(SUM(file_size), 0), AVG(file_size)").
		Row()
	if err := row.Scan(&totalSize, &avgSize); err != nil {
		return nil, err
	}
...
func NewDocumentChunkRepository(db *gorm.DB) DocumentChunkRepository {
	base := NewBaseRepository[models.DocumentChunk](db)
	return &DocumentChunkRepositoryImpl{
		BaseRepositoryImpl: base.(*BaseRepositoryImpl[models.DocumentChunk]),
	}
}
...
func (r *DocumentChunkRepositoryImpl) GetChunkStatistics(ctx context.Context, tenantID string) (*ChunkStats, error) {
	...
	var avgTokenCount *int64
	row := r.db.WithContext(ctx).Model(&models.DocumentChunk{}).
		Where("tenant_id = ?", tenantUUID).
		Select("COALESCE(SUM(token_count), 0), AVG(content_length), AVG(token_count)").
		Row()
	if err := row.Scan(&totalTokens, &avgChunkSize, &avgTokenCount); err != nil {
		return nil, err
	}
