...
func (cm *CredentialManager) encryptData(data []byte, key []byte, deriveInfo KeyDerivationInfo) ([]byte, EncryptionMetadata, error) {
	...
	case AlgorithmChaCha20Poly1305:
		...
		if chachaErr != nil {
			return nil, EncryptionMetadata{}, fmt.Errorf("failed to create ChaCha20Poly1305: %w", chachaErr)
		}
...
func (cm *CredentialManager) decryptData(encryptedData []byte, key []byte, meta EncryptionMetadata) ([]byte, error) {
	...
	case AlgorithmAES256GCM:
		...
		decrypted, err = aesgcm.Open(nil, meta.Nonce, encryptedData, nil)
		if err != nil {
	...
	case AlgorithmChaCha20Poly1305:
		aead, chachaErr := chacha20poly1305.New(key)
		if chachaErr != nil {
			return nil, fmt.Errorf("failed to create ChaCha20Poly1305: %w", chachaErr)
		}

		decrypted, err = aead.Open(nil, meta.Nonce, encryptedData, nil)
		if err != nil {
			return nil, fmt.Errorf("failed to decrypt with ChaCha20Poly1305: %w", err)
		}
...
func (km *InMemoryKeyManager) DeriveKey(ctx context.Context, masterKey []byte, keyInfo KeyDerivationInfo) ([]byte, error) {
	...
	case KDFArgon2ID:
		...
		time, _ := keyInfo.Params["time"].(float64)
		_, _ = keyInfo.Params["memory"].(float64)
		_, _ = keyInfo.Params["threads"].(float64)
		keyLen, _ := keyInfo.Params["key_length"].(float64)

		// For simplicity, using PBKDF2 as fallback
		return pbkdf2.Key(masterKey, keyInfo.Salt, int(time), int(keyLen), sha256.New), nil
