...
func TestTracingMiddleware(t *testing.T) {
	...
	next := http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		// Verify trace span is present in context (set by OpenTelemetry)
...
func TestRequestTimeoutMiddleware(t *testing.T) {
	...
	// The middleware sets a context deadline but doesn't forcefully cancel the handler,
	// so the handler's Sleep still runs to completion (~200ms).
