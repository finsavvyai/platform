...
func NewStructuredLogger(component, version string, config LoggingConfig) *StructuredLogger {
	...
	if config.Output == "file" && config.File != "" {
		...
		// SECURITY: Use restrictive permissions (0600) to prevent unauthorized access
		// Only the file owner can read and write to the log file
...
func (l *StructuredLogger) LogRequest(r *http.Request) {
	httpInfo := l.buildHTTPRequestInfo(r)
...
func (l *StructuredLogger) LogResponse(r *http.Request, resp *http.Response, duration time.Duration) {
	...
	httpInfo.ResponseTime = duration.String()
	if resp.ContentLength > 0 {
		httpInfo.BodySize = resp.ContentLength
	}
...
// buildHTTPRequestInfo builds HTTP info from request
func (l *StructuredLogger) buildHTTPRequestInfo(r *http.Request) *HTTPInfo {
	if r == nil {
		return nil
	}
