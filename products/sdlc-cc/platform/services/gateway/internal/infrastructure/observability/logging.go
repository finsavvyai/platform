...
// CorrelationContext holds correlation information for tracing
...
// Tracing types for middleware support
...
func (th *TraceHelper) SetAttributes(ctx context.Context, attrs map[string]interface{}) {
	th.logger.WithFields(attrs).Debug("Setting trace attributes")
}
...
// TraceHTTPResponse records HTTP response details
func (th *TraceHelper) TraceHTTPResponse(ctx context.Context, statusCode int, size int64, duration int) {
	...
	}).Debug("HTTP response traced")
}

// AddEvent adds an event to the current span
func (th *TraceHelper) AddEvent(ctx context.Context, name string, attrs map[string]interface{}) {
	th.logger.WithField("event", name).WithFields(attrs).Debug("Trace event")
}

// Shutdown gracefully shuts down the tracer provider
func (tp *TracerProvider) Shutdown(ctx context.Context) error {
	tp.logger.Info("Tracer provider shut down")
	return nil
}
...
func GetRequestID(r interface{}) string {
	if req, ok := r.(interface{ Header(string) string }); ok {
...
// SetRequestHeaders sets request headers for tracing
func SetRequestHeaders(r interface{}, ctx context.Context) {
	// No-op stub
}

// SetError sets error on current span
func SetError(ctx context.Context, component string, err error) {
	logrus.WithContext(ctx).WithError(err).WithField("component", component).Error("Span error")
}

// SetAttributes sets span attributes globally
func SetAttributes(ctx context.Context, category string, attrs map[string]interface{}) {
	logrus.WithContext(ctx).WithFields(attrs).WithField("category", category).Debug("Setting attributes")
}
