...
func TestCheck_BlocksAtHourLimit(t *testing.T) {
	rl, _ := newTestLimiter(t)
	// minute limit large enough not to trigger; hour limit = 2
	rl.SetTierConfig("hourly", TierConfig{
		Name: "hourly", RequestsPerMin: 1000, RequestsPerHour: 2,
		RequestsPerDay: 10000, ConcurrentLimit: 10, MaxPayloadBytes: 1024,
	})
	ctx := context.Background()

	for i := 0; i < 2; i++ {
		res, err := rl.Check(ctx, "t-h", "hourly", "global")
		require.NoError(t, err)
		assert.True(t, res.Allowed, "req %d allowed", i)
	}
	res, err := rl.Check(ctx, "t-h", "hourly", "global")
	require.NoError(t, err)
	assert.False(t, res.Allowed)
	assert.Equal(t, "per-hour", res.Window)
}

func TestCheck_BlocksAtDayLimit(t *testing.T) {
	rl, _ := newTestLimiter(t)
	// minute and hour limits large enough; day limit = 2
	rl.SetTierConfig("daily", TierConfig{
		Name: "daily", RequestsPerMin: 1000, RequestsPerHour: 1000,
		RequestsPerDay: 2, ConcurrentLimit: 10, MaxPayloadBytes: 1024,
	})
	ctx := context.Background()

	for i := 0; i < 2; i++ {
		res, err := rl.Check(ctx, "t-d", "daily", "global")
		require.NoError(t, err)
		assert.True(t, res.Allowed, "req %d allowed", i)
	}
	res, err := rl.Check(ctx, "t-d", "daily", "global")
	require.NoError(t, err)
	assert.False(t, res.Allowed)
	assert.Equal(t, "per-day", res.Window)
}

func TestMiddleware_RedisFailure_FailsOpen(t *testing.T) {
	mr := miniredis.RunT(t)
	client := redis.NewClient(&redis.Options{Addr: mr.Addr()})
	rl := NewTierRateLimiter(client)
	...
	// Kill Redis before the request so the limiter encounters an error.
	mr.Close()
	...
	// Fail-open: request should still pass through.
	assert.True(t, called, "handler must be called on Redis failure (fail-open)")
	assert.Equal(t, http.StatusOK, rec.Code)
}
