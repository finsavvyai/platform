...
func (m *MigrationManager) LoadMigrations() error {
	...
	// Two files sharing the same version prefix means the second is silently
	// skipped once the first is applied (schema_migrations PRIMARY KEY). Catch
	// this at load time so it cannot hide in production.
	if err := validateMigrations(m.migrations); err != nil {
		return fmt.Errorf("migration conflict: %w", err)
	}
...
// validateMigrations returns an error when two migrations share the same
// version number. Kept as a separate function so it can be unit-tested
// without a database connection.
func validateMigrations(migrations []Migration) error {
	seen := make(map[int]string, len(migrations))
	for _, mg := range migrations {
		if prev, dup := seen[mg.Version]; dup {
			return fmt.Errorf("version %d claimed by both %q and %q", mg.Version, prev, mg.Name)
		}
		seen[mg.Version] = mg.Name
	}
	return nil
}
