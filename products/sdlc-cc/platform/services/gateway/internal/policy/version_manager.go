...
func (vm *VersionManager) ActivateVersion(ctx context.Context, policyName, version, activatedBy string) error {
	...
	if chain != nil && chain.ActiveVersion != "" {
		...
		if err == nil {
			...
			if saveErr := vm.storage.SaveVersion(ctx, currentVersion); saveErr != nil {
				return fmt.Errorf("failed to deactivate current version %q: %w", chain.ActiveVersion, saveErr)
			}
		}
	}
...
func (vm *VersionManager) generateVersion(chain *PolicyVersionChain) string {
	...
	major := parts[0]
	minor := parts[1]
	_ = parts[2] // patch - unused for now

	// Simple increment - could be enhanced with semantic versioning logic
	return fmt.Sprintf("%s.%s.%s", major, minor, "0") // Reset patch for new changes
}
