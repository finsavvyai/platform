...
var (
	...
	ErrDeviceMismatch        = &TokenValidationError{Type: "device_mismatch", Message: "device fingerprint mismatch"}
)
...
func (s *jwtService) signToken(claims TokenClaims) (string, error) {
	...
	signed, err := jwt.Sign(token, jwt.WithKey(s.signingAlgorithm, s.signingKey))
	if err != nil {
...
func (s *jwtService) mapJWTError(err error) error {
	...
	// Check for validation errors using the library's typed errors first
	if jwt.IsValidationError(err) {
		switch {
		case strings.Contains(errMsg, `"exp" not satisfied`):
			return ErrTokenExpired
		case strings.Contains(errMsg, `"nbf" not satisfied`):
	...
	if strings.Contains(errMsg, "failed to verify signature") {
