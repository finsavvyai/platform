...
impl Config {
    ...
    fn parse_providers() -> Result<std::collections::HashMap<String, ProviderConfig>> {
        ...
        // OpenAI provider
        let openai_model_result: std::result::Result<String, env::VarError> =
            env::var("OPENAI_EMBEDDING_MODEL")
                .or_else(|_| Ok("text-embedding-ada-002".to_string()));
        if let (Ok(api_key), Ok(model)) = (env::var("OPENAI_API_KEY"), openai_model_result) {
            providers.insert(
                "openai".to_string(),
                ProviderConfig {
                    api_key,
        ...
        if let (Ok(api_key), Ok(model)) = (
            ...
            env::var("COHERE_EMBEDDING_MODEL")
                .or_else(|_| Ok::<String, env::VarError>("embed-english-v3.0".to_string())),
        ) {
            providers.insert(
                "cohere".to_string(),
                ProviderConfig {
