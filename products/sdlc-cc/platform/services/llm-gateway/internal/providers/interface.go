...
type Provider interface {
	...
	// GetConfig returns the provider configuration (for priority, etc.)
	GetConfig() models.ProviderConfig

	// GetLastHealthCheck returns when the provider was last health-checked.
	GetLastHealthCheck() time.Time
	// GetLastHealthStatus returns the last health check result, or nil.
	GetLastHealthStatus() *models.HealthStatus
}
...
// GetLastHealthCheck returns when the provider was last health-checked.
func (p *BaseProvider) GetLastHealthCheck() time.Time {
	return p.lastHealthCheck
}

// GetLastHealthStatus returns the last health check result, or nil.
func (p *BaseProvider) GetLastHealthStatus() *models.HealthStatus {
	return p.lastHealthStatus
}
