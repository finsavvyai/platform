...
import { ProgressBroadcaster } from '@/services/progress-broadcaster'
import { TenantClientRegistry } from '@/services/tenant-client-registry'
...
async function createServer(): Promise<FastifyInstance> {
  ...
  // ProgressBroadcaster needs its own subscriber connection because
  // ioredis won't multiplex regular commands with active subscriptions.
  ...
  // Day 11: subscribe to doc-progress pub/sub and fan out to connected
  // websocket clients filtered by tenant.
  const progressBroadcaster = new ProgressBroadcaster({
    redisSub,
    registry: new TenantClientRegistry(connectionManager),
    log: (level, msg, data) => {
      if (level === 'warn') server.log.warn({ msg, data })
      else server.log.info({ msg, data })
    },
  })
  await progressBroadcaster.start()
    ...
    try {
      ...
      connection.on('message', async (data: Buffer) => {
        try {
          ...
          server.log.error(`Error handling WebSocket message: ${error instanceof Error ? error.message : String(error)}`)
      ...
      server.log.error(`WebSocket authentication error: ${error instanceof Error ? error.message : String(error)}`)
  ...
  server.get('/api/collaboration/:sessionId/history', { preHandler: authenticate }, async (request: AuthenticatedRequest) => {
    ...
    const { documentId } = request.query as any
    const history = await collaborationEngine.getSessionHistory(sessionId, documentId)
    return { history }
  })
  ...
  const gracefulShutdown = async () => {
    ...
    await progressBroadcaster.stop()
