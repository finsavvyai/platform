...
export class CollaborationEngine {
  ...
  async updateCursor(sessionId: string, _documentId: string, userId: string, cursor: Omit<ParticipantCursor, 'userId' | 'timestamp'>): Promise<void> {
  ...
  async getActiveSession(documentId: string): Promise<CollaborationSession | null> {
    ...
    for (const [_sessionId, sessionData] of Object.entries(sessions)) {
      const session = JSON.parse(sessionData)
  ...
  private applyOperationToState(state: { content: string; version: number }, operation: CollaborationOperation): { content: string; version: number } {
    let { content } = state
  ...
  private async notifySessionParticipants(sessionId: string, message: unknown, excludeUserId?: string): Promise<void> {
