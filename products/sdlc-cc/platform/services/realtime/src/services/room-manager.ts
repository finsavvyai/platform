...
export class RoomManager {
  ...
  async isUserInRoom(roomId: string, userId: string): Promise<boolean> {
    const result = await this.redis.sismember(`room:${roomId}:participants`, userId)
    return result === 1
  }

  private async notifyRoomParticipants(roomId: string, message: unknown, _excludeUserId?: string): Promise<void> {
